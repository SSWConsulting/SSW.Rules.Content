...
SSW Rules title convention:
- Titles MUST start with "Do you" and end with a question mark (e.g. "Do you use X for Y?")
- This is a strict site-wide convention — do not deviate from it

Requirements for EACH title:
- Follow the "Do you ... ?" convention exactly
- Under 60 characters total (including "Do you" and "?")
- No surrounding quotes
- Unique — every title in your response must be different from the others
- Specific and concrete — capture what makes this rule distinct from the others in the group
    ...
    for attempt in range(1, max_retries + 1):
        ...
        for item in parsed:
            ...
            if len(title) > _MAX_TITLE_CHARS:
                ...
                valid = False
                break
            if not title.lower().startswith("do you"):
                print(f"  [ai] Title does not follow 'Do you...' convention: {title!r}")
                valid = False
                break
            if not title.endswith("?"):
                print(f"  [ai] Title does not end with '?': {title!r}")
                valid = False
                break
            result[fp] = title

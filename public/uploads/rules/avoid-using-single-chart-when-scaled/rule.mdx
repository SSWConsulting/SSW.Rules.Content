---
archivedreason: null
authors:
- title: Jeoffrey Fischer
  url: https://ssw.com.au/people/jeoffrey-fischer
categories:
- category: categories/company-operations/rules-to-better-reporting-services.mdx
created: 2023-12-11 14:38:33+00:00
guid: 85eec401-261c-4054-82eb-f73c9d951160
redirects: []
related:
- rule: public/uploads/rules/customization-do-you-know-which-version-of-sql-reporting-services-and-visual-studio-you-are-using/rule.mdx
seoDescription: Avoid using a single chart when you need it to be scaled, instead
  use an embedded chart within the table for dynamic chart lists.
title: Data Layout - Do you avoid using a single chart when you need it to be scaled?
type: rule
uri: avoid-using-single-chart-when-scaled
---

<endOfIntro />

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePreset="badExample"
  figureText={"Bad example - Just a chart - poor scaling for only 1 record"}
  src="/uploads/rules/avoid-using-single-chart-when-scaled/RulesToBetterBusinessIntelligence_ChartBad1.gif"
/>


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePreset="badExample"
  figureText={"Bad example - Just a chart - poorly scaling when many records"}
  src="/uploads/rules/avoid-using-single-chart-when-scaled/RulesToBetterBusinessIntelligence_ChartBad2.gif"
/>


The reason for this problem is that the 'size' property of the chart control doesn't support expressions like 'Count(Rows) or queried values like 'Fields!RowCount.Value', so the chart control cannot adjust its size according to the data.

The solution for this problem is to use an embedded chart within the table - this will create a dynamic chart list similar to the list shown below.


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePreset="default"
  figureText={"Size property of the chart control"}
  src="/uploads/rules/avoid-using-single-chart-when-scaled/ChartProperties_size.jpg"
/>


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePreset="goodExample"
  figureText={"Good example - A table with chart"}
  src="/uploads/rules/avoid-using-single-chart-when-scaled/RulesT12.gif"
/>


To do this, you need to create a table in your report and add a chart into each of the rows.


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePreset="default"
  figureText={"Embedded chart in a table will generate dynamic chart list"}
  src="/uploads/rules/avoid-using-single-chart-when-scaled/RulesToRS-chart-1.gif"
/>


**Note:** When rendering a report to your browser or an email, Reporting Services generates a separate image for every single image in the report, even if they are identical. When you are using graphs, images or charts in your report, this can cause a large number of images to be generated. Always include a red warning at the top of any emailed reports so that users do not try and forward or reply to them. Use a warning like this:

<span style="color:red">**Warning:** Do not reply to or forward this report in an email - Outlook may slow down or even hang.</span>
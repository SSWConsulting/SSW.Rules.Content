...
4. If the tool fails, use the `dotnet ef dbcontext scaffold` CLI as a fallback (prefer the current LTS EF Core, e.g. EF Core 10, or EF Core 8).
5. Implement interface from step 1 and refactor entities
...
6. Update namespaces (for Entities, EF Core namespaces and removing legacy namespaces)
...
13. Optional: Upgrade to the current LTS .NET (e.g. .NET 10) if you’re on .NET Framework or older .NET
14. Optional: Upgrade to the current LTS EF Core (e.g. EF Core 10) if you started on an older EF Core version
15. Test, test, test...

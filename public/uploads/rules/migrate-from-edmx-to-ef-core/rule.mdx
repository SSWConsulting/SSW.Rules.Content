...
lastUpdated: 2026-02-03T06:29:24.623Z
...
1. Abstract existing `ObjectContext/Entities` class with a custom interface (e.g. `ITenantDbContext`)
2. Reverse-engineer the database (scaffold)
  - UI (preferred): [EF Core Power Tools](https://marketplace.visualstudio.com/items?itemName=ErikEJ.EFCorePowerTools)
  - CLI (fallback): `dotnet ef dbcontext scaffold` (prefer the current LTS EF Core, e.g. EF Core 10, or EF Core 8)
3. Implement the interface from step 1 and refactor entities
  ...
  3. Address common refactor gotchas (see "Common gotchas" below): nullability differences, lazy loading, query translation, and performance
  4. Add a **TechDebt** comment and PBI when you need to defer deeper refactors - [Do you know the importance of paying back Technical Debt?](/technical-debt/)
4. Update namespaces (for Entities, EF Core namespaces and removing legacy namespaces)
5. Remove EF6 namespaces like `System.Data.Entity` from files that are now using EF Core (otherwise you can get confusing type clashes and LINQ/runtime issues)
6. Add `Microsoft.EntityFrameworkCore` namespace
7. Update dependency injection
   1. Use modern `.AddDbContext()` or `.AddDbContextPool()`
8. Update migration strategy (from DB-first to Code-first)
9. Adopt EF Core Migrations (`dotnet ef migrations add` / `dotnet ef database update`) or your preferred migration approach; DbUp can still be useful, but it’s no longer “the default” once you’re Code-First
10. Remove EDMX completely (can be done sooner if migration is done in 1 go rather than in steps)
11. Verify thoroughly (test, test, test...)
...
## Common gotchas (EF6/EDMX → EF Core)

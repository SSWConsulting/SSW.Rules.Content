---
archivedreason: Redundant https://my.sugarlearning.com/SSW/items/13879/design-shared-folders
authors:
- author: adam-stephensen
- author: adam-cogan
- author: rebecca-liu
created: 2014-04-03 20:10:19+00:00
createdBy: Tiago Araujo
createdByEmail: TiagoAraujo@ssw.com.au
guid: c44339bd-9159-4b44-8495-a92a42ed763c
isArchived: true
lastUpdated: 2016-12-09 20:16:50+00:00
lastUpdatedBy: Tiago Araujo
lastUpdatedByEmail: TiagoAraujo@ssw.com.au
redirects: []
seoDescription: Learn how to share media files easily between workers while ensuring
  central backup and offline access.
title: Do you know how to share media files?
type: rule
uri: do-you-know-how-to-share-media-files
---
It is important to be able to easily share files between workers. It is also important that all content is backed up centrally in case an employee is unavailable.

Ideally SharePoint would be used for all content storage but occasionally it is not ideal.

<endIntro />

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="bad"
  figure="SharePoint Explorer View requires waiting every time you save a file while it syncs back to the server"
  src="/uploads/rules/do-you-know-how-to-share-media-files/share-media-files-1.jpg"
/>


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="bad"
  figure="OneDrive (was SkyDrive) Pro enables offline access and saves locally and then syncs back to the SharePoint server asynchronously, but requires the whole document library to be brought to the local computer, even if you only need one folder in the document library"
  src="/uploads/rules/do-you-know-how-to-share-media-files/share-media-files-2.jpg"
/>



<boxEmbed
  style="greybox"
  body={<>
    **Suggestion for OneDrive (was SkyDrive) Team #1 :** Enable OneDrive to have selective sync on folders within a document library.
  </>}
  figurePrefix="none"
  figure=""
/>

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="bad"
  figure="Files shared via SkyDrive are hard to back up centrally. When you share folders through OneDrive (was SkyDrive) they are only made available through the web interface"
  src="/uploads/rules/do-you-know-how-to-share-media-files/share-media-files-3.jpg"
/>



<boxEmbed
  style="greybox"
  body={<>
    **Suggestion to OneDrive (was SkyDrive) Team #2:** When folders are shared with another OneDrive user, the shared folder should appear in the recipients OneDrive folder. This allows it to be backed up by a central user at HQ.
  </>}
  figurePrefix="none"
  figure=""
/>

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="good"
  figure="DropBox allows offline access. When you share a DropBox folder with another Dropbox user, the shared folder appears in the DropBox folder on their machine with a different icon to indicate sharing"
  src="/uploads/rules/do-you-know-how-to-share-media-files/share-media-files-4.jpg"
/>


Having the folder on the remote machine allows remote backup. All folders used for SSW Work must be shared with the SSW Dropbox Account.
The SSW sys-admin is responsible for backing up the SSW Dropbox account daily.


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="none"
  figure="To allow you to use Dropbox for work, first create a folder called SSW_[YourName] (e.g. SSW_AdamStephensen), right click on the folder and choose Share this folder..."
  src="/uploads/rules/do-you-know-how-to-share-media-files/share-media-files-5.jpg"
/>



<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="none"
  figure="Add the email address of the company Dropbox account and click Send Invites. Once the Administrator accepts the share, your important work files will be available in the case that you leave or get hit by a bus"
  src="/uploads/rules/do-you-know-how-to-share-media-files/share-media-files-6.jpg"
/>



<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="none"
  figure="The Administrator account must now accept the sharing invitation, and the folder will be added to the Administrators Dropbox folder. The Admin should configure a machine to pull the files locally and back them up"
  src="/uploads/rules/do-you-know-how-to-share-media-files/share-media-files-7.jpg"
/>
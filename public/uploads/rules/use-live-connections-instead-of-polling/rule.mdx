...
| **SSE** | Server → Client | Simple notifications, live feeds, dashboards (no binary data) |
| **Webhooks** | Server → Server | Service-to-service communication, third-party integrations |

### When to use SignalR

SignalR is the recommended choice for .NET applications. It automatically selects the best transport (WebSockets → SSE → Long Polling) and provides:
...
```
  ...
  figurePrefix="bad"
  figure="Polling wastes resources even when nothing has changed"
/>
...
```
  ...
  figurePrefix="good"
  figure="SignalR pushes updates only when data actually changes"
/>
...
```
  ...
  figurePrefix="good"
  figure="SSE provides a lightweight option for unidirectional streaming"
/>
...
<boxEmbed
  body={<>
    **Tip:** Don't over-engineer. If users only need updates every few minutes and real-time isn't critical, a simple refresh button or periodic polling with proper caching might be the right choice.
  </>}
  figurePrefix="none"
  figure=""
  style="tips"
...
* **Near real-time is acceptable** - A 30-second delay doesn't impact the user experience (e.g., email inbox, social media feeds)
* **The app is internal with few users** - 5 users polling every 10 seconds is just 30 requests/minute - hardly a scaling concern
* **You're building a prototype or MVP** - Get it working first, optimize later when you have real usage data
* **Infrastructure doesn't support WebSockets** - Some corporate proxies, firewalls, or legacy systems block WebSocket connections
...
1. **Do users need updates instantly (< 1 second)?** → Use SignalR or WebSockets
...
```
  ...
  figurePrefix="good"
  figure="Configure a backplane when scaling SignalR horizontally"
/>

...
<boxEmbed
  ...
  body={<>
    ...
    Walk the tree. The list of projects and what each one covers is small, so it **fits with room to spare**. And picking the best match means weighing every project against the others, not retrieving the three that look closest - so it **needs the whole corpus at once**.

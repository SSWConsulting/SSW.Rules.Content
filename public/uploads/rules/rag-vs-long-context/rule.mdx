...
flowchart
 ...
 Start(["Start"]) --> Fits{"Corpus fits\nthe context window?"}

 Fits -->|"No"| Handles{"Exact identifiers, live data\nor per-user permissions?"}
 Fits -->|"Yes"| Global{"Answer needs the\nwhole corpus at once?"}

 Handles -->|"Yes"| Agentic(["Agentic search"])
 Handles -->|"No"| RAG(["Vector retrieval (RAG)"])

 Global -->|"Yes"| LongContext(["Long context"])
 Global -->|"No"| Repeats{"Same documents\non every request?"}

 Repeats -->|"Yes"| LongContext
 Repeats -->|"No"| RAG
```

| | Long context | Vector retrieval (RAG) | Agentic search |
| --- | --- | --- | --- |
| **Corpus size** | Has to fit the window | Unbounded | Unbounded |

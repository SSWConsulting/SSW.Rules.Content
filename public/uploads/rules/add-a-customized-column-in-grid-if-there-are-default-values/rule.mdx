---
seoDescription: When adding default values to a grid, consider including a "(customized)" column to easily identify changes from the defaults
type: rule
title: Do you add a "(customized)" column in grid if there are default values?
categories:
  - category: categories/software-engineering/rules-to-better-windows-forms-applications.mdx
uri: add-a-customized-column-in-grid-if-there-are-default-values
authors:
  - title: Adam Cogan
    url: https://ssw.com.au/people/adam-cogan/
created: 2024-03-26T21:36:47.000Z
guid: 4ed43f37-6443-43ad-b201-308c17e3d7c0
lastUpdated: 2024-06-11T00:27:00.000Z
lastUpdatedBy: Brady Stroud [SSW]
lastUpdatedByEmail: bradystroud@ssw.com.au
createdBy: Igor Goldobin
createdByEmail: igor.goldobin@gmail.com
isArchived: false
---

Add a column and show "(customized)" in grid - that is an easier way to know if you have changed from the defaults.

<endOfIntro />

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="badExample"
  captionText={"Bad example - You need to compare with the default values to know whether it is modified"}
  src="/uploads/rules/add-a-customized-column-in-grid-if-there-are-default-values/badexampleforcustomizedcolumn.gif"
/>


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="goodExample"
  captionText={"Good example - A \"(customized)\" column is an easier way to know if you have changed from the defaults"}
  src="/uploads/rules/add-a-customized-column-in-grid-if-there-are-default-values/goodexampleforcustomizedcolumn.gif"
/>

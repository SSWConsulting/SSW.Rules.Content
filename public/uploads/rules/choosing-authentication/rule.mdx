...
[Better Auth](https://better-auth.com) is a comprehensive, framework-agnostic authentication framework for TypeScript that emerged in 2024 as the spiritual successor to [NextAuth.js / Auth.js](https://authjs.dev). In 2025, the Auth.js team officially joined the Better Auth project, consolidating the JavaScript/TypeScript open-source auth ecosystem under one roof. Better Auth is self-hosted, fully open-source (MIT), and built from the ground up with end-to-end TypeScript type safety in mind.

Unlike IdentityServer (which targets .NET), Better Auth is designed for JavaScript and TypeScript stacks — Next.js, SvelteKit, Nuxt, Express, Hono, and more. It ships with email/password auth, social logins, 2FA (TOTP/SMS), organisation/team management, and an extensible plugin system out of the box, without requiring third-party packages for common scenarios. If you are coming from NextAuth.js, an official migration guide is provided.
...
* Built-in 2FA (TOTP/SMS), organisation management, passkeys, magic links, and social logins out of the box
* Extensible plugin ecosystem (admin panel, API keys, multi-tenancy, etc.)
* Auth.js/NextAuth.js team now maintains Better Auth — clear migration path from NextAuth
...
* No managed/cloud-hosted option — unlike Auth0, there is no hosted SaaS tier
* Relatively new (2024) — smaller community than Auth0, IdentityServer, or Entra
...
* You need built-in 2FA or organisation management without additional configuration

...
lastUpdated: 2026-07-24T07:19:41.493Z
...
1\. Deactivate, don't delete — set Status = Inactive (statecode) rather than removing the record.

2\. Tag what you touch — mark the affected records with an identifiable marker and run the change under a dedicated service account, so they're easy to filter, review and reverse.

3\. Get approval — present the draft list of records for sign-off before any hard delete.

4\. Delete only what's approved — run the built-in Bulk Record Deletion job with tight filters, e.g. Status == Inactive AND ModifiedBy == \[service account] AND ModifiedOn == today.

❌ Bad example

Running an ad-hoc script that hard-deletes straight from production with no deactivate step, no marker, and no approval list — leaving nothing to review or roll back. (This is what happened at CP Cards.)

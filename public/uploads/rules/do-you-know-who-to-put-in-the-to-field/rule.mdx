---
archivedreason: null
authors:
- title: Martin Hinshelwood
  url: https://ssw.com.au/people/martin-hinshelwood
- title: Adam Cogan
  url: https://ssw.com.au/people/adam-cogan
categories:
- category: categories/communication/rules-to-better-email.mdx
created: 2011-04-25 07:13:08+00:00
createdBy: SSW2000\cindywang
createdByEmail: CindyWang@ssw.com.au
guid: 292307eb-59d1-453e-be8c-f83e8dd0368f
isArchived: false
lastUpdated: 2021-02-26 22:37:06.196000+00:00
lastUpdatedBy: Tiago Araújo [SSW]
lastUpdatedByEmail: tiagov8@gmail.com
redirects: []
related:
- rule: public/uploads/rules/include-names-as-headings/rule.mdx
seoDescription: Ensure task assignment clarity by explicitly adding recipients' names
  to the "To -" field in emails.
title: Do you know who to put in the "To:" field?
type: rule
uri: do-you-know-who-to-put-in-the-to-field
---

Make sure that anyone tasked in emails is explicitly added to the "To:" line. This helps them identify emails where they have tasks.

A fair percentage of people open an email and don’t read who is on the "To:" or "Cc:" lines.

<endOfIntro />

<emailEmbed
  from=""
  to="@SSWScrumDogs"
  cc=""
  bcc=""
  subject="Northwind App - Update work items"
  shouldDisplayBody={true}
  body={<>
    ## Hi All

Can everyone make sure to update your work items as you work on them?

## Hi Mark

1. At the end of the day, please double check the above was done
  </>}
  figurePreset="badExampleExample"
  figureText={"Bad example - Mark can’t search for tasks that have been assigned to him"}
/>

<emailEmbed
  from=""
  to="@SSWScrumDogs, Mark"
  cc=""
  bcc=""
  subject="Northwind App - Update work items"
  shouldDisplayBody={true}
  body={<>
    ## Hi All

Can everyone make sure to update your work items as you work on them?

## Hi Mark

1. At the end of the day, please double check the above was done
  </>}
  figurePreset="goodExampleExample"
  figureText={"Good example - Mark can filter his emails based on whether his name is on the \"To:\" field"}
/>
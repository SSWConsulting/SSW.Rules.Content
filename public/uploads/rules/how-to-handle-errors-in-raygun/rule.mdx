---
archivedreason: null
authors:
- title: Adam Cogan
  url: https://www.ssw.com.au/people/adam-cogan
- title: Eric Phan
  url: https://www.ssw.com.au/people/eric-phan
created: 2016-12-30 03:34:53+00:00
createdBy: Jernej Kavka
createdByEmail: JernejKavka@ssw.com.au
guid: ffd7161f-7cac-435d-ae26-8ec42651b59b
isArchived: false
lastUpdated: 2021-09-17 17:06:54.084000+00:00
lastUpdatedBy: Tiago Araújo [SSW]
lastUpdatedByEmail: tiagov8@gmail.com
redirects:
- do-you-know-how-to-handle-errors-in-raygun
seoDescription: Learn how to handle errors in Raygun and improve your application's
  health with our step-by-step guide.
title: Do you know how to handle errors in Raygun?
categories:
  - category: categories/project-delivery/rules-to-better-devops.mdx
  - category: categories/software-engineering/rules-to-better-raygun.mdx
type: rule
uri: how-to-handle-errors-in-raygun
---

Your team should always be ensuring that the health of the application is continually improving.

The best way to do that is to check the exceptions that are being logged in the production application. Every morning, fix the most serious bug logged over the last week. After it is fixed then email yesterday's application health to the Product Owner.

<endIntro />

There's traditional error logging software like Log4Net or Elmah, but they just give you a wall of errors that are duplicated and don't give you the ability to mark anything as complete. You'll need to manually clear out the errors and move them into your task tracking system (Azure DevOps/VisualStudio.com).

This is where RayGun or Application Insights comes into the picture. RayGun gives you the following features:

- Grouping exceptions
- Ignoring/filtering exceptions
- Triaging exceptions (mark them as resolved)
- Integrations to TFS/VisualStudio.com to create a Bug, Slack
- Tracking the exceptions to a deployment
- See which errors are occurring the most often

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="bad"
  figure="Elmah gives you a wall of exceptions and no way to flag exceptions as completed"
  src="/uploads/rules/how-to-handle-errors-in-raygun/elmah.gif"
/>


<emailEmbed
  from=""
  to="Adam"
  cc=""
  bcc=""
  subject="Raygun Health Check for TimePro"
  shouldDisplayBody={true}
  body={<>
    ## Hi Adam

Please find below the Raygun Health Check for TimePro:

![Figure: Raygun health check for TimePro in the past 7 days](/uploads/rules/how-to-handle-errors-in-raygun/Raygun-health-check-for-TimePro-in-the-past-7-days.jpg)

![Figure: Resolved issues in the past 7 days](/uploads/rules/how-to-handle-errors-in-raygun/2.png)

![Figure: The next issue to be worked on](/uploads/rules/how-to-handle-errors-in-raygun/3.jpg)

&lt; This email is as per [https://www.ssw.com.au/rules/how-to-handle-errors-in-raygun](/how-to-handle-errors-in-raygun) &gt;
  </>}
  figurePrefix="good"
  figure="Email with Raygun application health report"
/>
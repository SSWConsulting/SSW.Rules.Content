---
archivedreason: null
authors:
- title: Adam Cogan
  url: https://ssw.com.au/people/adam-cogan
- title: Brendan Richards
  url: https://ssw.com.au/people/brendan-richards
categories:
- category: categories/software-engineering/rules-to-better-application-performance.mdx
created: 2019-09-16 23:34:58+00:00
createdBy: Tiago Araujo
createdByEmail: TiagoAraujo@ssw.com.au
guid: 8fe322ef-32ef-4338-9a2c-1148e52e7f9d
isArchived: false
lastUpdated: 2019-09-17 00:02:10+00:00
lastUpdatedBy: Tiago Araujo
lastUpdatedByEmail: TiagoAraujo@ssw.com.au
redirects:
- do-you-know-the-highest-hit-pages
seoDescription: Measuring website usage helps optimize high-traffic pages for better
  system performance.
title: Do you know the highest hit pages?
type: rule
uri: know-the-highest-hit-pages
---

Measuring which pages receive the most hits will help a great deal when looking to optimise your website.

- Measuring usage will help to determine the current business value of a page or feature
- Optimising a highly used page will have a higher impact on overall system performance

A number of great tools exist to find the highest hit pages.

<endOfIntro />


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  captionText={"Application Insights can return request counts under the performance screen"}
  src="/uploads/rules/know-the-highest-hit-pages/App-Insights-return-request.png"
/>



<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  captionText={"Google Analytics provides powerful usage statistics"}
  src="/uploads/rules/know-the-highest-hit-pages/GoogleAnalytics-Stats.png"
/>
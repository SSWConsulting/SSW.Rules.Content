---
archivedreason: null
authors:
- title: Adam Cogan
  url: https://ssw.com.au/people/adam-cogan
- title: Ulysses Maclaren
  url: https://ssw.com.au/people/ulysses-maclaren
- title: Greg Harris
  url: https://ssw.com.au/people/greg-harris
categories:
- category: categories/communication/rules-to-better-calendars.mdx
created: 2020-11-10 18:24:52+00:00
createdBy: Tiago Araujo
createdByEmail: TiagoAraujo@ssw.com.au
guid: bce2f455-3c5a-41c0-9775-200ac1963379
isArchived: false
lastUpdated: 2021-05-24 20:38:39.597000+00:00
lastUpdatedBy: Tiago Araújo [SSW]
lastUpdatedByEmail: tiagov8@gmail.com
redirects:
- appointments-do-you-automate-scheduling-of-meetings
related:
- rule: public/uploads/rules/appointments-throw-it-in-their-calendar/rule.mdx
- rule: public/uploads/rules/collaborate-across-timezones/rule.mdx
- rule: public/uploads/rules/automated-webinars-for-multiple-time-zones/rule.mdx
seoDescription: Automate meeting scheduling effortlessly with AI-powered solutions
  like Outlook 365's Scheduling Poll and FindTime for faster, stress-free coordination
title: Appointments - Do you automate scheduling of meetings?
type: rule
uri: automate-schedule-meetings
---

We've all been part of that email chain... _"Let's meet to discuss this"_, _"When are you free?"_, _"I can do Friday"_, _"Friday's no good, what about Monday?"_, _"I'm free in the morning"_, _"Penny can't make that, how about Tuesday afternoon after 3?"_, etc.

AI can make scheduling meetings with multiple attendees much easier.

<endOfIntro />

**Note:** The following solutions work best when you're dealing with people in your own organization, as they then have access to all calendar data, but with a bit of back and forth, they can still work for externals too. There are some options.

## Scheduling Poll in Microsoft Outlook 365

You can do complex scheduling tasks and calendar polls with this built-in Outlook 365 feature.


<youtubeEmbed url="https://www.youtube.com/embed/bMmOjJrlqPA" description={"Video: How to use Scheduling Poll in Microsoft Outlook 365 📆 (3 min)"} />



<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  captionText={"Voting options on Scheduling Poll"}
  src="/uploads/rules/automate-schedule-meetings/schedule-poll-screenshot.png"
/>


## FindTime

This option is best if you want to be selective about a few options for when to meet, and send a poll to attendees to choose.

FindTime shows what days and times work the best for you and the attendees when available using free/busy data.

Propose your selected times to all attendees and allow everyone to vote and quickly come to a consensus... and you’re done!

FindTime sends out the meeting invite on your behalf. More on [https://support.microsoft.com/en-gb/office/install-findtime-2501ff38-0a47-4dc5-999f-bcca329f0f63](https://support.microsoft.com/en-gb/office/install-findtime-2501ff38-0a47-4dc5-999f-bcca329f0f63)

**Note:** Findtime’s competitor is [https://calendly.com](https://calendly.com)


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  captionText={"FindTime gives a nice UI for people working in the same org, as you can see times that might be available for all attendees easily (e.g. 8 am above)"}
  src="/uploads/rules/automate-schedule-meetings/findtime-scheduler.jpg"
/>



<boxEmbed
  style="greybox"
  body={<>
    **Mobile – does not work ❌**
As of Dec 2020, FindTime does not work on iOS.

![Figure: FindTime - No mobile support for iOS](/uploads/rules/automate-schedule-meetings/findtime-mobile.jpg)
  </>}
  captionStyle="default"
  captionText={""}
/>
---
archivedreason: null
authors:
- title: Adam Cogan
  url: https://ssw.com.au/people/adam-cogan
categories:
- category: categories/design/rules-to-better-interfaces-winforms-controls.mdx
created: 2012-11-27 09:21:07+00:00
createdBy: Rebecca Liu
createdByEmail: RebeccaLiu@ssw.com.au
guid: c632ba59-8d03-48e9-a02b-aaad31dd875e
isArchived: false
lastUpdated: 2017-02-23 14:38:12+00:00
lastUpdatedBy: Tiago Araujo
lastUpdatedByEmail: TiagoAraujo@ssw.com.au
redirects:
- controls-do-you-extend-the-size-of-your-comboboxes-to-show-as-many-results-as-possible-(windows-forms-only)
seoDescription: Controls expertly extend the size of your ComboBoxes to show as many
  results as possible, optimizing user experience while maintaining a balance between
  display and form space.
title: Controls - Do you extend the size of your ComboBoxes to show as many results
  as possible? (Windows Forms Only)
type: rule
uri: controls-do-you-extend-the-size-of-your-comboboxes-to-show-as-many-results-as-possible-windows-forms-only
---

When designing your form, it's a good idea to help your user whenever it's possible. So it's a good idea to extend your ComboBoxes to show as many results as possible to save your user from scrolling. Also, you should extend the width of the dropdown in order to show the longest items.

<endOfIntro />

However, you should not extend your ComboBox without limit, normally the maximum number of items should be under 10 and the maximum width of the drop-down should be smaller than your hosting form.

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePreset="badExample"
  figureText={"Bad example - You have to scroll to see all the result, and the long results are cut off"}
  src="/uploads/rules/controls-do-you-extend-the-size-of-your-comboboxes-to-show-as-many-results-as-possible-windows-forms-only/ComboBox-Size-1.jpg"
/>


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePreset="goodExample"
  figureText={"Good example - The size of the drop down has been extended to allow user to see as much as possible"}
  src="/uploads/rules/controls-do-you-extend-the-size-of-your-comboboxes-to-show-as-many-results-as-possible-windows-forms-only/ComboBox-Size-2.jpg"
/>


Changing the maximum items is easy, just include the following code in your form:

```cs
cbxOUList.MaxDropDownItems = cbxOUList.Items.Count;

// Changing the drop down size is a bit of tricky

Graphics g = Graphics.FromHwnd(this.Handle);

SizeF stringSize = new SizeF();
stringSize = g.MeasureString(longString, cbx.Font, 600);
int adjustedSize = cbx.DropDownWidth;

if (adjustedSize < (int) stringSize.Width) {
  adjustedSize = (int) stringSize.Width;
}

cbx.DropDownWidth = adjustedSize;
```
---
authors:
- title: Anastasia Cogan
  url: https://www.ssw.com.au/people/anastasia-cogan
- title: Marina Gerber
  url: https://www.ssw.com.au/people/marina-gerber
- title: Luke Cook
  url: https://www.ssw.com.au/people/luke-cook/
categories:
- category: categories/company-operations/rules-to-better-office-operations.mdx
created: 2025-11-03 21:47:42+00:00
createdBy: Tiago Araújo [SSW]
createdByEmail: tiagov8@gmail.com
guid: 83e6cbf2-7333-453e-931d-a6f8c3562e4f
isArchived: false
lastUpdated: 2025-11-03 21:47:42+00:00
lastUpdatedBy: Tiago Araújo [SSW]
lastUpdatedByEmail: tiagov8@gmail.com
redirects:
- report-defects-to-property-manager
related:
- rule: public/uploads/rules/supervise-tradespeople/rule.mdx
- rule: public/uploads/rules/watch-do-you-know-what-is-going-on/rule.mdx
seoDescription: Always include key details when reporting a defect to the Property
  Manager — clear, complete reports improve turnaround time, communication, and follow-up.
tips: ''
title: Do you know the best way to report an office defect?
type: rule
uri: report-defects
---

When a defect occurs in an office (e.g. a broken coffee machine, faulty air conditioning, or leaking tap), it’s crucial to report it with enough detail so that the Property Manager can take quick and accurate action.  Many issues get delayed or mishandled because the initial report is missing key details like **location**, **urgency**, or **who has been informed**.

<endOfIntro />


<youtubeEmbed url="https://www.youtu.be/AxBS6JYcLIQ" description={"Video: YakShaver - fix the broken Coffee Machine ☕️ ❌ (1 min)"} />


Good reporting not only helps the Property Manager resolve issues faster, it also improves **turnaround time (TAT)**, **feedback**, and **follow-up communication**.

When reporting a defect, always make sure your message is **clear**, **complete**, and **actionable**.

## Use a backlog

* **Create a PBI (Product Backlog Item)** in your maintenance backlog titled **“Defect –  &#123;&#123; BRIEF DESCRIPTION &#125;&#125;"**
* Add a note to the broken item (e.g. sticky note) with the PBI number as per the rule: [Office environment - Do you label broken equipment?](https://www.ssw.com.au/rules/label-broken-equipment)

  This is so that others can track progress and updates.\
  **Example:** *“Created PBI: [Defect – Broken coffee machine in SSW France kitchen](https://github.com/SSWConsulting/SSW.Maintenance/issues/1)”*


<boxEmbed
  style="info"
  body={<>
    **Tip:** Use a tool like [YakShaver](https://yakshaver.ai) on your phone to create PBIs in seconds! This will ensure your issues are logged, tracked, and resolved efficiently - saving time, reducing confusion, and improving everyone’s work environment.
  </>}
  captionStyle="default"
  caption={""}
/>


### Key details to be included

#### 1. Location

Start broad and zoom in:

* Office name → floor → area → exact item\
  **Example:** *“SSW Brisbane→ First floor → Kitchen area → The counter next to the sink”*

#### 2. Urgency and impact

Does the defect affect your **productivity** or **safety**?  If yes:

* Clearly state the impact\
  **Example:** *“Can’t use meeting room due to flickering lights”*
* Mention any temporary fix you’ve applied\
  **Example:** *“Taped over the exposed wire until repaired”*

#### 3. Ensure you inform responsible parties

Inform the people who is aware of the problem. Always include:

* The **Head of Office**
* The **Property Manager**


<boxEmbed
  style="info"
  body={<>
    **Tip:** Use @mentions in the PBI, or if you’re reporting via Teams or email, e.g. *“@John Smith @Sarah Jones”*.
  </>}
  captionStyle="default"
  caption={""}
/>


#### 4. Historic information

* Note any past occurrences or related maintenance\
  **Example:** *"Coffee Machine keeps jamming; it was repaired in last month with the same issue"*


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  caption={"What information should you include when reporting a defect"}
  src="/uploads/rules/report-defects/ssw-rules-best-way-to-report-an-office-defect-v3.jpg"
/>


- - -

## Examples

<emailEmbed
  from="Hugo"
  to="Jean"
  cc=""
  bcc=""
  subject="Coffee machine is broken"
  shouldDisplayBody={true}
  body={<>
    ## Hi Jean

The coffee machine is broken. Can you call someone to fix it?
  </>}
  captionStyle="badExample"
  caption={"Bad example - Emails are easily lost or forgotten. The coffee machine will never get fixed!"}
/>

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="goodExample"
  caption={"Good example- Clear and includes all the essential details for fast action and tracking"}
  src="/uploads/rules/report-defects/defects-pbi-example.png"
/>


Consistency in reporting helps ensure that all offices communicate defects in the same clear way - improving turnaround time, transparency, and accountability.
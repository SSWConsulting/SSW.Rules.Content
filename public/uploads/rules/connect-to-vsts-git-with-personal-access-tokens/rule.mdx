---
archivedreason: null
authors:
- author: adam-stephensen
- author: jernej-kavka
created: 2017-07-03 05:46:30+00:00
createdBy: Jernej Kavka
createdByEmail: JernejKavka@ssw.com.au
guid: 1ed929b8-b0a7-4bec-850b-21ec76be4752
isArchived: false
lastUpdated: 2017-09-04 12:41:22+00:00
lastUpdatedBy: Jernej Kavka
lastUpdatedByEmail: JernejKavka@ssw.com.au
redirects:
- do-you-know-to-connect-to-vsts-git-with-personal-access-tokens
seoDescription: Connect to VSTS Git with Personal Access Tokens, automate your workflow
  and secure access to Visual Studio Online projects.
title: Do you know to connect to VSTS Git with Personal Access Tokens
categories:
- category: categories/project-delivery/rules-to-better-version-control-with-git.mdx
type: rule
uri: connect-to-vsts-git-with-personal-access-tokens
---
When you create a new git repository and need to push it to VSTS you need to provide login credentials.

It isn't always clear how to do this.

<endIntro />

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="bad"
  figure="Alternate  Authentication Credentials should not be used. When you change the password it invalidates all projects and can't be scoped to limit access to your Team Services data"
  src="/uploads/rules/connect-to-vsts-git-with-personal-access-tokens/vsts-alternative-login.png"
/>


Instead, you should use Personal Access Token. You can do this in two ways.

The first option is to make sure your Git for Windows is up-to-date and when cloning the repository, you use Microsoft Account to log in. Personal Access Token for Git will be created for you.

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="good"
  figure="Windows for Git credential manager will automatically create Personal Access Token for Git"
  src="/uploads/rules/connect-to-vsts-git-with-personal-access-tokens/git-credentials-personal-access-token.png"
/>


Option 2 is to manually create Personal Access Token and use it as a password for Git login.

You can follow this blog post for full instructions: [Using Personal Access Tokens to access Visual Studio Online](https://roadtoalm.com/2015/07/22/using-personal-access-tokens-to-access-visual-studio-online/).

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="good"
  figure="You can also manually enter Personal Access Token into password section if the credential manager doesn't work"
  src="/uploads/rules/connect-to-vsts-git-with-personal-access-tokens/git-credentials-personal-access-token-manual.png"
/>
---
archivedreason: null
authors:
- title: Adam Cogan
  url: https://www.ssw.com.au/people/adam-cogan
created: 2020-07-28 21:54:30+00:00
createdBy: Tiago Araujo
createdByEmail: TiagoAraujo@ssw.com.au
guid: 68843a19-8840-4ac3-a095-88e3898034d2
isArchived: false
lastUpdated: 2020-07-28 21:57:30+00:00
lastUpdatedBy: Tiago Araujo
lastUpdatedByEmail: TiagoAraujo@ssw.com.au
redirects:
- do-you-know-to-use-lowercase-after
seoDescription: Use lowercase after " - " to ensure proper formatting and readability.
title: Do you know to use lowercase after "-"?
categories:
  - category: categories/communication/rules-to-better-technical-documentation.mdx
type: rule
uri: use-lowercase-after-a-dash
---

Use lowercase after "-" in a text.

<endIntro />


<boxEmbed
  style="greybox"
  body={<>
    “Make a change to the content of the system you're currently looking at - <mark style="background-color: #FEF08A">I</mark>f you don't know it is called SugarLearning, then we have a real problem :)”
  </>}
  figurePrefix="none"
  figure=""
/>
**❌ Figure: Bad example - Uppercase after "-"**  


<boxEmbed
  style="greybox"
  body={<>
    “Make a change to the content of the system you're currently looking at - <mark style="background-color: #FEF08A">i</mark>f you don't know it is called SugarLearning, then we have a real problem :)”
  </>}
  figurePrefix="none"
  figure=""
/>
**✅ Figure: Bad example - lowercase after "-"**
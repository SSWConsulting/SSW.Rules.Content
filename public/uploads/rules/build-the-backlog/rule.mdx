---
archivedreason: null
authors:
- title: Martin Hinshelwood
  url: https://ssw.com.au/people/martin-hinshelwood
- title: Adam Cogan
  url: https://ssw.com.au/people/adam-cogan
- title: Greg Harris
  url: https://ssw.com.au/people/greg-harris
- title: Matt Goldman
  url: https://ssw.com.au/people/matt-goldman
categories:
- category: categories/project-delivery/rules-to-better-scrum-using-azure-devops.mdx
- category: categories/project-delivery/rules-to-better-scrum-using-github.mdx
- category: categories/project-delivery/rules-to-better-scrum.mdx
created: 2020-09-24 03:52:11+00:00
createdBy: GregHarris
createdByEmail: GregHarris@ssw.com.au
guid: fa1cee83-0211-413a-99cf-ea6a9438e59f
isArchived: false
lastUpdated: 2024-06-08 03:27:17+00:00
lastUpdatedBy: Brady Stroud [SSW]
lastUpdatedByEmail: bradystroud@ssw.com.au
redirects:
- product-owner-do-you-know-how-to-update-the-backlog
related:
- rule: public/uploads/rules/github-issue-templates/rule.mdx
- rule: public/uploads/rules/do-you-have-templates-for-your-pbis-and-bugs/rule.mdx
- rule: public/uploads/rules/turn-emails-into-pbis/rule.mdx
seoDescription: Does your Product Owner know how to maintain a backlog effectively
  by recording conversations as Product Backlog Items (PBIs) or Issues?
title: Backlog - Does your Product Owner know how to maintain the backlog?
type: rule
uri: build-the-backlog
---

All good Scrum teams have a backlog. The backlog is built by taking a conversation and recording it as one or more **Product Backlog Items** (PBIs) (e.g. Azure DevOps) or **Issues** (e.g. GitHub, JIRA).

You should [create PBIs during or straight after the conversation](/report-bugs-and-suggestions), rather than using emails that may never be entered into the backlog.


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  caption={"Get typing during a conversation to make the meeting tangible"}
  src="/uploads/rules/build-the-backlog/Typing.jpg"
/>


The Product Owner is responsible for owning the Product Backlog. See the video on ["Do you know how to be a good Product Owner?"](/do-you-know-the-how-to-be-a-good-product-owner)

Although these requirements come from the Product Owner, it is often the developers who will record these PBIs.

<endOfIntro />

#### 1. Emails

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="badExample"
  caption={"Bad example - This is a poor way to add a PBI/Issue as you can’t order emails by the business priority"}
  src="/uploads/rules/build-the-backlog/EmailBadExample.png"
/>


#### GitHub Issues (Recommended)

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="goodExample"
  caption={"Good example - Github Issues"}
  src="/uploads/rules/build-the-backlog/GitHubIssues.png"
/>


#### 3. Azure DevOps - E.g. [https://ssw.visualstudio.com](https://ssw.visualstudio.com)

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="goodExample"
  caption={"Good example - Azure DevOps allows you to enter an item into the backlog, in any priority order"}
  src="/uploads/rules/build-the-backlog/AzureDevOpsBacklog_1710232021931.png"
/>


**What's next?**

* [Discuss the backlog](/discuss-the-backlog)
* [What to do when it's done?](/dones-do-you-know-how-to-do-a-perfect-done-replying-to-a-bug)
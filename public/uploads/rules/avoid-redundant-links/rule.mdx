---
archivedreason: null
authors:
- title: Adam Cogan
  url: https://www.ssw.com.au/people/adam-cogan
- title: Tiago Araujo
  url: https://www.ssw.com.au/people/tiago-araujo
created: 2015-02-16 02:15:22+00:00
guid: 8e22e80c-b922-4986-ac4b-3ac49707a941
redirects:
- do-you-avoid-redundant-linking-(aka-single-link-to-single-location)
- do-you-avoid-redundant-linking-aka-single-link-to-single-location
related:
- rule: public/uploads/rules/less-is-more/rule.mdx
- rule: public/uploads/rules/do-you-avoid-linking-a-page-to-itself/rule.mdx
seoDescription: Avoid redundant linking (aka single link to a single location) by
  having only one link per page section that points to the same target. This helps
  users focus on the main text content and reduces confusion caused by multiple links
  leading to the same place.
title: Do you avoid redundant linking (aka have a single link to a single location)?
categories:
  - category: categories/design/rules-to-better-websites-navigation.mdx
type: rule
uri: avoid-redundant-links
---

Often a page has duplicated links pointing to the same page - all fairly close to each other, like in 1 or 2 paragraphs. It is a good idea to **avoid** having multiple links to the exact same page section, because they:

* Distract users from reading the main text content
* Confuse users as they have to decide which link to click first (and then end up to the same place anyway)
* Annoy users when they are repeatedly directed to the same page

<endIntro />

<boxEmbed
  style="info"
  body={<>
    **Note:** The exception is when you find useful to users to have a second link in a different section of the page (e.g. 1 in the content, 1 in the "Related" section).
  </>}
  figurePrefix="none"
  figure=""
/>

<boxEmbed
  style="greybox"
  body={<>
    [SSW Web Hosting](https://www.ssw.com.au) offers competitive, high performance [hosting plans](https://www.ssw.com.au). We use Windows Servers with the latest security patches and SQL Server. Our Data center has multiple connections to major backbones, and our support staff are Microsoft Certified. [View more about our hosting plans](https://www.ssw.com.au).
  </>}
  figurePrefix="bad"
  figure="Redundant links nearby pointing to the same page"
/>

<boxEmbed
  style="greybox"
  body={<>
    [SSW Web Hosting](https://www.ssw.com.au) offers competitive, high performance hosting plans. We use Windows Servers with the latest security patches and SQL Server. Our Data center has multiple connections to major backbones, and our support staff are Microsoft Certified.
  </>}
  figurePrefix="good"
  figure="Single link makes text more clear and readable"
/>

...
lastUpdated: 2026-02-10T05:48:11.897Z
...
To invoke this skill, type `/review` or `/project:review` in Claude Code, `@review` in GitHub Copilot, or simply ask a question that matches the skill's description and the agent will load it automatically.

For connecting to external systems like databases or APIs, use [MCP servers](/rules/mcp-servers-for-context) instead.

## When to use skills

You don't always need to write skills from scratch. Trusted providers like [Anthropic](https://github.com/anthropics/skills) publish pre-built skills you can install. For example, Anthropic's official skills repository includes document skills (DOCX, PDF, PPTX, XLSX), creative and design skills, and development utilities. In Claude Code, you can install them via the plugin system:
...
Create a `SKILL.md` file inside a named directory under `.claude/skills/` (or `.claude/commands/` which still works):

```
.claude/
  skills/
    review/
      SKILL.md       → invoked with /review
    commit/
...
| ---------- | --------------------------------------------------- | -------------------- |
| Personal   | `~/.claude/skills/` or `~/.config/opencode/skills/` | All your projects    |
| Project    | `.claude/skills/` in the repo                       | This project only    |
| Enterprise | Managed settings (org-wide deployment)              | All users in the org |
...
* **Reference skills in your README** - mention available skills in your project's README so contributors know what's available (e.g. "Run `/review` for a code review" or "Use `/deploy` to deploy to staging")

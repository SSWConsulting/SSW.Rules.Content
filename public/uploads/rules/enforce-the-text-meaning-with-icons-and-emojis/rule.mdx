---
archivedreason: null
authors:
- title: Adam Cogan
  url: https://ssw.com.au/people/adam-cogan
- title: Tiago Araujo
  url: https://ssw.com.au/people/tiago-araujo
categories:
- category: categories/communication/rules-to-better-email.mdx
- category: categories/design/rules-to-better-interfaces-general-usability-practices.mdx
- category: categories/design/rules-to-better-websites-layout-and-formatting.mdx
created: 2014-12-16 18:16:16+00:00
guid: 5f5645eb-dda4-421b-84e7-c51b76887205
redirects:
- do-you-use-icons-to-enforce-the-text-meaning
- do-you-use-icons-in-web-pages-to-enforce-the-text-meaning
- use-icons-webpages
- use-icons-to-enforce-the-text-meaning
related:
- rule: public/uploads/rules/best-libraries-for-icons/rule.mdx
seoDescription: Using icons and emojis to enforce text meaning can be a simple yet
  effective way to draw attention to important information, clarify unclear requirements,
  and enhance user understanding.
title: Do you use icons/emojis to enforce the text meaning?
type: rule
uri: enforce-the-text-meaning-with-icons-and-emojis
---

People may not pay attention to some important words in your interface. Adding a simple and clear icon beside the words will make the difference.

For emails and web content, using an simple emoji is an easy and friendly way to achieve the same result 🙂.

<endOfIntro />

#### Using icons

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="badExample"
  captionText={"Bad example - No icons to indicate the status"}
  src="/uploads/rules/enforce-the-text-meaning-with-icons-and-emojis/validation-bad.jpg"
/>


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="goodExample"
  captionText={"Good example - Green tick and red cross help the user to know what's going on"}
  src="/uploads/rules/enforce-the-text-meaning-with-icons-and-emojis/validation-good.jpg"
/>


#### Using emojis


<boxEmbed
  style="greybox"
  body={<>
    I join a lot of Sprint Reviews, and there is a consistent problem I see among Scrum teams. The PBIs have limited or missing information. Usually, this is due to unclear requirements...
  </>}
  captionStyle="badExample"
  captionText={"Bad example - No emojis to enforce the meaning"}
/>



<boxEmbed
  style="greybox"
  body={<>
    I join a lot of Sprint Reviews, and there is a consistent problem I see among Scrum teams. The PBIs have limited or missing information 😥. Usually, this is due to unclear requirements...
  </>}
  captionStyle="goodExample"
  captionText={"Good example - The emoji gives extra focus on what is important"}
/>


---

See this rule being used on different scenarios:

- [Adding a green tick for successful messages or a red cross for error messages](/messages-do-you-use-green-tick-red-cross-and-spinning-icon-to-show-the-status) (before the text)
- [Adding the type of file next to download links](/use-icons-to-not-surprise-users) (before the text)
- [Adding an icon to external links](/do-you-make-external-links-clear) (after the text)
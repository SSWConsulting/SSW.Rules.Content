---
archivedreason: null
authors: []
categories:
- category: categories/communication/rules-to-better-powerpoint-presentations.mdx
created: 2010-06-08 10:12:59+00:00
createdBy: RebeccaLiu
createdByEmail: RebeccaLiu@ssw.com.au
guid: e42c5ded-bdfd-474f-8074-5af4f6be4213
isArchived: false
lastUpdated: 2024-06-08 03:27:17+00:00
lastUpdatedBy: Brady Stroud [SSW]
lastUpdatedByEmail: bradystroud@ssw.com.au
redirects: []
seoDescription: Use large images on slides to ensure clear visibility from the back
  row and prioritize important text.
title: Do you use the full slide for screenshots?
type: rule
uri: do-you-use-the-full-slide-for-screenshots
---

Small images are hard to see. Remember your audience sitting at the back of the room. Especially for screenshots displaying important text, use all your real estate.

<endOfIntro />

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="badExample"
  caption={"Bad example - the image doesn't cover the whole slide"}
  src="/uploads/rules/do-you-use-the-full-slide-for-screenshots/smallbad.jpg"
/>


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="goodExample"
  caption={"Good example - Cover the whole slide with your image to make it easier for people to see from the back row"}
  src="/uploads/rules/do-you-use-the-full-slide-for-screenshots/goodbig.jpg"
/>
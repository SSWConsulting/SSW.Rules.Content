---
archivedreason: null
authors:
- title: Adam Cogan
  url: https://ssw.com.au/people/adam-cogan
- title: Cameron Shaw
  url: https://ssw.com.au/people/cameron-shaw
- title: Ulysses Maclaren
  url: https://ssw.com.au/people/ulysses-maclaren
- title: Tiago Araujo
  url: https://ssw.com.au/people/tiago-araujo
categories:
- category: categories/communication/rules-to-better-email.mdx
- category: categories/project-delivery/rules-to-better-scrum.mdx
created: 2009-12-10 06:55:30+00:00
guid: ed0fa76a-418b-4b59-9e3b-2544c08b910e
redirects:
- dones-do-your-done’s-include-a-url
- dones-do-your-dones-include-a-url
- include-links-in-emails
related:
- rule: public/uploads/rules/add-context-reasoning-to-emails/rule.mdx
- rule: public/uploads/rules/include-useful-details-in-emails/rule.mdx
seoDescription: Include URLs in tasks and "Done" emails to enhance transparency and
  collaboration by providing easy access to relevant information.
title: Do you include URLs in "Done" tasks and emails?
type: rule
uri: include-links-in-dones
---

Always include the relevant URL to your emails, like when you want to request or just made a change to a webpage or document. This way people can easily check the details of the tasks. This is especially important for ["Done" emails](/reply-done-and-delete-the-email).

If you are using a task tracking system like **Azure DevOps**, **GitHub**, or Jira, also include the link to the PBI/Issue/task/PR for extra information.

<endOfIntro />


<boxEmbed
  variant="info"
  body={<>
    **Tip:** It's [important to give context and reasoning](/do-you-add-context-reasoning-to-your-emails) to your emails.
  </>}
  figurePreset="defaultExample"
  figureText={"XXX"}
/>



<boxEmbed
  variant="greybox"
  body={<>
    Done
  </>}
  figurePreset="badExampleExample"
  figureText={"Bad example - How can we check the task was done correctly?"}
/>



<boxEmbed
  variant="greybox"
  body={<>
    Done - northwind&#46;com/about-us

  </>}
  figurePreset="goodExampleExample"
  figureText={"Good example - Easy to check what was done"}
/>



<boxEmbed
  variant="greybox"
  body={<>
    Done - northwind&#46;com/about-us/

As requested on ssw2&#46;visualstudio&#46;com/Northwind/\_workitems/edit/00001
  </>}
  figurePreset="goodExampleExample"
  figureText={"Good example - Easy to check what was done + includes the context of the task within the Sprint"}
/>


## Ensure changes are live

Before closing a PBI/Issue and declaring a task 'done' with a link, ensure that your changes are live and accessible for verification.


<boxEmbed
  variant="info"
  body={<>
    **Note:** It is the PR author's responsibility to [avoid merge debt](/merge-debt) by getting the PR reviewed ASAP.
  </>}
  figurePreset="defaultExample"
  figureText={"XXX"}
/>


### Scenario: PR waiting for approval


<boxEmbed
  variant="greybox"
  body={<>
    Done - ssw&#46;com&#46;au/rules/dones-is-your-inbox-a-task-list-only
  </>}
  figurePreset="badExampleExample"
  figureText={"Bad example - Link is included but changes are not live yet"}
/>



<boxEmbed
  variant="greybox"
  body={<>
    Done - github&#46;com/SSWConsulting/SSW.Rules&#46;Content/pull/0001

  </>}
  figurePreset="badExampleExample"
  figureText={"Bad example - Using the PR link instead of the final page link"}
/>



<boxEmbed
  variant="greybox"
  body={<>
    (PR waiting for approval - github&#46;com/SSWConsulting/SSW.Rules.Content/pull/0001)

Done - Changes will be on ssw&#46;com&#46;au/rules/dones-is-your-inbox-a-task-list-only

  </>}
  figurePreset="okExampleExample"
  figureText={"OK example - Changes are not live yet, but people are aware and have the links"}
/>


### Scenario: PR is approved and merged


<boxEmbed
  variant="greybox"
  body={<>
    Done - ssw&#46;com&#46;au/rules/dones-is-your-inbox-a-task-list-only
  </>}
  figurePreset="goodExampleExample"
  figureText={"Good example - Final link is included and changes are live to be checked"}
/>


## Ensure others have permissions

It is a common problem where someone CC'd will not have permissions to see a file and the sender knows this. You should still add the link but inform the recipient.

### Scenario: Recipient doesn't have permissions


<boxEmbed
  variant="greybox"
  body={<>
    Done - onedrive&#46;live.com/file-name.xls
  </>}
  figurePreset="badExampleExample"
  figureText={"Bad example - Link is included but recipient won't be able to open it, potentially generating more emails"}
/>



<boxEmbed
  variant="greybox"
  body={<>
    (link for reference - you don't have permissions)

Done - onedrive&#46;live.com/file-name.xls
  </>}
  figurePreset="goodExampleExample"
  figureText={"Good example - Link is included and people are aware of permission issues"}
/>


**Video:** [Top 10+ Rules to Better Email Communication with Ulysses Maclaren](https://www.youtube.com/watch?v=LAqRokqq4jI)
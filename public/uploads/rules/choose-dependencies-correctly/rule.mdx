...
<youtubeEmbed url="https://www.youtube.com/embed/lqZo4waMB3c?si=Ceoy1y1h9mNmU7PX" description="Video: The NPM Worm of November 2025 (7 min)" />
...
<imageEmbed alt="Image" size="medium" showBorder={false} figurePrefix="bad" figure="Last commit was years ago!" src="/uploads/rules/choose-dependencies-correctly/activity-bad-npm.png" />

<imageEmbed alt="Image" size="medium" showBorder={false} figurePrefix="good" figure="More recent activity on the package" src="/uploads/rules/choose-dependencies-correctly/date-fns-recent-commits.png" />
...
<imageEmbed alt="Image" size="large" showBorder={true} figurePrefix="bad" figure="Weekly downloads is much lower than substitute packages" src="/uploads/rules/choose-dependencies-correctly/popularity-npm-bad.png" />

<imageEmbed alt="Image" size="large" showBorder={true} figurePrefix="good" figure="A lot more popular, battle-tested 💪" src="/uploads/rules/choose-dependencies-correctly/popularity-npm-good.png" />
...
<imageEmbed alt="Image" size="large" showBorder={true} figurePrefix="bad" figure="Released ages ago!" src="/uploads/rules/choose-dependencies-correctly/last-published-npm-bad.png" />

<imageEmbed alt="Image" size="large" showBorder={true} figurePrefix="good" figure="More recent release" src="/uploads/rules/choose-dependencies-correctly/last-published-npm-good.png" />
...
<imageEmbed alt="Image" size="large" showBorder={true} figurePrefix="good" figure="Licensing mentioned" src="/uploads/rules/choose-dependencies-correctly/licensing-good.png" />

### 9. Does it have known vulnerabilities?

Check for known vulnerabilities before you install a package, not after. Your tooling can often do this for you:

* **.NET** - NuGet has built-in vulnerability auditing, so installing or restoring a vulnerable package will often raise build warnings (e.g. `NU1902`, `NU1903`)
* **JavaScript** - run `npm audit` (or `pnpm audit` / `yarn npm audit`) after adding a package

Don't ignore these warnings. If a package already has open advisories, pick a patched version or a different package. See [monitoring for dependency vulnerabilities](/monitor-packages-for-vulnerabilities) to keep checking after it's installed.
...
Future developers will have better context and will be able to make a better decision should there be any situational or business requirement changes.

The best way to record the decision to add a new package is with an [Architectural Decision Record (ADR)](/architectural-decision-records). ADRs capture the context, the options you considered, and why you chose one over the others. A [package audit log](/package-audit-log) is also a great way to keep a running list of every package decision.

<imageEmbed alt="Image" size="large" showBorder={false} figurePrefix="good" figure="A markdown file should include your reasons to assist future developers" src="/uploads/rules/choose-dependencies-correctly/md-decision-reasons.png" />

---
seoDescription: Avoid putting connection strings in business module and instead consolidate
  them into a single file for easier management and deployment.
type: rule
title: Do you avoid putting connection strings in Business module?
categories:
- category: categories/software-engineering/rules-to-better-connection-strings.mdx
uri: avoid-putting-connection-strings-in-business-module
authors:
- author: alumni
created: 2024-03-09 06:05:50.030000+00:00
guid: f3141c85-0cc8-40da-a08b-68f01bf6595f
---
We strongly recommend that putting all connection strings together and saving them to one file, it is convenient for management and deployment.

<endIntro />

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="bad"
  figure="Putting connection strings in Business module"
  src="/uploads/rules/avoid-putting-connection-strings-in-business-module/2024-03-09_16-10-24.png"
/>

---
authors:
- title: Adam Cogan
  url: https://www.ssw.com.au/people/adam-cogan
categories:
- category: categories/communication/rules-to-better-email.mdx
created: 2023-12-01 16:23:48+00:00
createdBy: Tiago Araújo [SSW]
createdByEmail: tiagov8@gmail.com
guid: 02a35dc4-5c79-47b9-a561-8f4d2fe81363
isArchived: false
lastUpdated: 2025-09-18 12:04:17+00:00
lastUpdatedBy: Chloe Lin [SSW]
lastUpdatedByEmail: chloelin@ssw.com.au
related:
- rule: public/uploads/rules/how-to-share-a-file-folder-in-sharepoint/rule.mdx
- rule: public/uploads/rules/change-link-sharing-default-behaviour/rule.mdx
seoDescription: Office365 users avoid using the 'Share' functionality with tasks/questions
  to ensure important emails are noticed and read.
title: Office365 - Do you avoid using 'Share' functionality with tasks/questions?
type: rule
uri: avoid-using-share-functionality
---

Sharing a file is easy when looking at a file. However the email people get from it will have a notification-look, which may result in it being overlooked/ignored.

Because of that, you should avoid using that functionality if you want people to notice and read your email. A normal email with the link and instructions should be sent instead.

<endOfIntro />

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="badExample"
  captionText={"Bad example - Sending an email with a file using the 'Share' functionality"}
  src="/uploads/rules/avoid-using-share-functionality/share-screenshot.jpg"
/>


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="badExample"
  captionText={"Bad example - The email sent has a notification-look meaning it may be ignored"}
  src="/uploads/rules/avoid-using-share-functionality/share-email-screenshot.png"
/>


<emailEmbed
  from="Adam"
  to="Gordon"
  cc=""
  bcc=""
  subject="Invoices in June&July 2019 - Do we need this?"
  shouldDisplayBody={true}
  body={<>
    ## Hi Gordon

See this file: &#123;&#123; URL &#125;&#125;

1. Do you need this anymore?
2. If not zz it

-a
  </>}
  captionStyle="goodExample"
  captionText={"Good example - Link and tasks sent in a regular email"}
/>


<boxEmbed
  style="info"
  body={<>
    **Note:** It is a good idea to review SharePoint and Teams stats and tell users **not** to use this funcionality by pointing them to this rule.
  </>}
  captionStyle="default"
  captionText={""}
/>
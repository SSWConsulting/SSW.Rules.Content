---
archivedreason: Deprecated - Currently we deploy with PowerShell and Infrastructure-As-Code
  via platforms like GitHub and AzureDevOps.
authors:
- title: Adam Stephensen
  url: https://ssw.com.au/people/adam-stephensen
created: 2013-02-06 18:57:43+00:00
createdBy: Tiago Araujo
createdByEmail: TiagoAraujo@ssw.com.au
guid: ed2c4951-62af-4e56-8e98-67c53a5fb67d
isArchived: false
lastUpdated: 2015-11-27 17:06:43+00:00
lastUpdatedBy: Tiago Araujo
lastUpdatedByEmail: TiagoAraujo@ssw.com.au
redirects: []
seoDescription: Deploy to multiple environments with PowerShell and Infrastructure-As-Code,
  including staging and production.
title: Do you Deploy to other Environments?
type: rule
uri: do-you-deploy-to-other-environments
---

Once you have configured “Build Once, Deploy Many” you can open the folder for any build on the build server and deploy any build, to any environment.

<endOfIntro />

This means that if you deploy version 37, and then want to roll back to version 36 simply open the folder for 36 and re-run that batch file (assuming there has been no breaking DB schema changes).


<boxEmbed
  variant="todo"
  body={<>
    TODO: AdamS – Write a rule on non-breaking Schema changes
  </>}
  figurePreset="defaultExample"
  figureText={"XXX"}
/>



<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePreset="default"
  figureText={"To deploy to staging and production, open the appropriate drops folder and execute the correct batch file from the drops folder"}
  src="/uploads/rules/do-you-deploy-to-other-environments/deploy-other-environments.jpg"
/>
---
archivedreason: Outdated - TFS is outdated and has been replaced by Azure DevOps.
  See https://www.ssw.com.au/rules/rules-to-better-devops/
authors:
- title: Mehmet Ozdemir
  url: https://www.ssw.com.au/people/mehmet-ozdemir
created: 2013-06-24 06:39:56+00:00
createdBy: Mehmet Ozdemir
createdByEmail: MehmetOzdemir@ssw.com.au
guid: 6f76bbd9-96da-4e4a-8023-f4bcedc6d51d
isArchived: true
lastUpdated: 2013-08-23 02:39:42+00:00
lastUpdatedBy: Jacky Zhou
lastUpdatedByEmail: jackyzhou@ssw.com.au
redirects:
- do-you-know-how-to-correctly-use-the-terms-configuration-and-customization-in-the-tfs-context
seoDescription: Correctly using "configuration" and "customization" terms in TFS context
  refers to changes made to application settings versus adapting TFS to fit a customized
  process.
title: 'Do you know how to correctly use the terms: Configuration and Customization
  in the TFS context?'
categories:
  - category: categories/project-delivery/rules-to-better-tfs-administration.mdx
type: rule
uri: tfs-configuration-vs-customization
---

It is important to use consistent language when talking about changes made to a TFS environment. The term configuration and customization are sometimes used interchangeably. It is clearer to use the following to differentiate them:

<endIntro />

**Configuration** applies to any changes made to application settings that help to make your TFS installation work correctly for a particular environment

Configuration would apply to objects like: TFS Administration Console (SharePoint, Microsoft Test Manager (MTM), Visual Studio, Web Access, Lab Management integration), User Security, Check-in options.

**Customization** is when TFS is changed to fit into a customized process.

Customization would apply to: Changing Work Item Templates, Custom Reports, Workflow and using the API to connect to other systems.

**Extending** is a term used with CRM that includes developing Plug-in's. With TFS this is covered under Customization.

See the [CRM](/crm-configuration-vs-customization-vs-extension) rule for correct definitions.
---
archivedreason: null
authors:
- title: Adam Cogan
  url: https://ssw.com.au/people/adam-cogan
categories:
- category: categories/communication/rules-to-better-technical-documentation.mdx
- category: categories/design/rules-to-better-websites-layout-and-formatting.mdx
created: 2021-07-14 22:51:02.119000+00:00
createdBy: Brady Stroud
createdByEmail: bradystroud@me.com
guid: db3afa6c-23e6-4b25-ac47-80e3945fb555
isArchived: false
lastUpdated: 2021-07-14 23:30:45.215000+00:00
lastUpdatedBy: Tiago Araújo [SSW]
lastUpdatedByEmail: tiagov8@gmail.com
related:
- rule: public/uploads/rules/right-format-to-show-phone-numbers/rule.mdx
- rule: public/uploads/rules/the-value-of-consistency/rule.mdx
seoDescription: Format your addresses consistently using the structure - Number, Street
  Name, City, State (abbreviation) Postal Code, Country
title: Do you know how to format addresses?
type: rule
uri: address-formatting
---

Use a consistent format when writing addresses.

The structure should follow: **Number, Street Name, City, State (abbreviation) Postal Code, Country**

* Beware of the commas positioning (inexisting between **State** and **Postal Code**)
* Don't use dashes, slashes, or bars to separate the elements (OK if it is in the **Street Name** part)
* **Country** is not always necessary depending on the audience
* If you have enough space, it is OK to write it in 2 lines
* We're in Australia and this should work for most countries' addresses, but some specific locations might have different address structures that won't allow following this rule

<endOfIntro />


<boxEmbed
  variant="greybox"
  body={<>
    **Level 1, 81 - 91 Military Road | Neutral Bay - NSW, 2089 AUS**
  </>}
  figurePreset="badExampleExample"
  figureText={"Bad example - SSW main office address not following the standard address formatting"}
/>



<boxEmbed
  variant="greybox"
  body={<>
    **Level 1/81-91 Military Rd, Neutral Bay, NSW 2089, Australia**
  </>}
  figurePreset="goodExampleExample"
  figureText={"Good example - SSW main office address following the standard address formatting"}
/>
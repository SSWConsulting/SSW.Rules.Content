---
archivedreason: null
authors: []
categories:
- category: categories/infrastructure-and-networking/rules-to-better-sharepoint.mdx
created: 2012-02-27 16:26:00+00:00
createdBy: TiagoAraujo
createdByEmail: TiagoAraujo@ssw.com.au
guid: c31997cb-b93b-42a7-8c6c-41efeea1d0db
isArchived: false
lastUpdated: 2025-09-18 12:04:17+00:00
lastUpdatedBy: Chloe Lin [SSW]
lastUpdatedByEmail: chloelin@ssw.com.au
redirects: []
seoDescription: Delete orphaned users from SharePoint's People Picker and keep your
  site organized with a simple URL and a few clicks.
title: Do you know how to delete orphaned users from the People Picker?
type: rule
uri: do-you-know-how-to-delete-orphaned-users-from-the-people-picker
---

The SharePoint "People Picker" is a great tool for quickly selecting users of your SharePoint site. It allows you to quickly select users from your organization by browsing Active Directory.

<endOfIntro />


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  caption={"The People Picker"}
  src="/uploads/rules/do-you-know-how-to-delete-orphaned-users-from-the-people-picker/PeoplePicker.jpg"
/>


Unfortunately, if you have added users directly to your site collection, and later deleted or disabled them from Active Directory, you will notice that these orphaned users will still appear in your People Picker. This will eventually clutter up your People Picker.


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  caption={"A user in the People Picker that was deleted from Active Directory"}
  src="/uploads/rules/do-you-know-how-to-delete-orphaned-users-from-the-people-picker/PeoplePicker-deleted.jpg"
/>



<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  caption={"Searching in Active Directory for the user shows the user is in fact deleted"}
  src="/uploads/rules/do-you-know-how-to-delete-orphaned-users-from-the-people-picker/PeoplePicker-searching.jpg"
/>


To remove these orphaned users, as a SharePoint Administrator, you can open the following URL (where "www.northwind.com" is your SharePoint URL):

* `http://www.northwind.com/\_catalogs/users/sample.aspx`

On this page you will find a list of all the users that are members of your site collection, including the orphaned users.

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  caption={"We have found our orphaned user!"}
  src="/uploads/rules/do-you-know-how-to-delete-orphaned-users-from-the-people-picker/PeoplePicker-found.jpg"
/>


To remove the user simply click on the **Username** | Click **Delete User from Site Collection** . This will instantly remove the user from the People Picker.
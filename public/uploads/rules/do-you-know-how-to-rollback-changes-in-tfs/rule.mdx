archivedreason: Outdated (Legacy) - TFS rollback guidance (TFVC). For modern Git undo/revert workflows, see [Do you know these important git commands?](/important-git-commands).
authors:
- noimage: true
  title: David Klein
  url: https://www.ssw.com.au/people/david-klein
- title: Justin King
  url: https://www.ssw.com.au/people/justin-king
- noimage: true
  title: Ryan Tee
  url: https://www.ssw.com.au/people/ryan-tee
- title: Tristan Kurniawan
  url: https://www.ssw.com.au/people/tristan-kurniawan
created: 2011-11-18 03:52:54+00:00
createdBy: System Account
createdByEmail: Unknown@ssw.com.au
guid: 6980541f-5084-4ef7-b997-51d90f4e6885
isArchived: true
lastUpdated: 2014-05-07 04:03:38+00:00
lastUpdatedBy: System Account
lastUpdatedByEmail: Unknown@ssw.com.au
redirects: []
seoDescription: Rollback changes in TFS by undoing pending changes or using Team Foundation
  Server Power Tools to revert unwanted code revisions.
title: Do you know how to rollback changes in TFS?
categories:
  - category: categories/project-delivery/rules-to-better-version-control-with-tfs-aka-source-control.mdx
type: rule
uri: do-you-know-how-to-rollback-changes-in-tfs
---

This field should not be null (Remove me when you edit this field).

<endIntro />

There are two ways to do this:

**A.** If you haven’t checked in any files since you started modifying them then the process is simple:

* Right click your solution and **Undo Pending Changes**

  ![](/uploads/rules/do-you-know-how-to-rollback-changes-in-tfs/rollback1.gif)

**B.** If you aren’t so lucky and have made some commits along the way then the only option is to use the Rollback command:

To use this you will need to install Team Foundation Server Power Tools v1.2

* Find the revision before you started checking code in using the **History command**

  
<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="none"
  figure="The last revision before Tristan made changes was 5367"
  src="/uploads/rules/do-you-know-how-to-rollback-changes-in-tfs/rollback2.gif"
/>


* Open the Command Prompt in your current working directory and type **“c:\Program Files\Microsoft Team Foundation Server Power Tools\tfpt.exe” rollback /changeset:5367**

  ![](/uploads/rules/do-you-know-how-to-rollback-changes-in-tfs/rollback3.gif)

* Click  **Yes** and the rollback will proceed

---

It would be nice if there was a GUI for this tool so that I can just right click and select rollback.

See [Better Software Suggestion – TFS](https://www.ssw.com.au/archive/standards/better-software-suggestions/team-foundation-server.html#RollbackGUI)
---
seoDescription: Find recent files quickly on Bing by logging into your work account or viewing your colleagues' files.
type: rule
archivedreason:
title: Do you know the best way to find your (or others) recent files?
guid: 16c0e275-a0da-4e39-8a3b-579d431a56d6
uri: the-best-way-to-find-recent-files
created: 2020-07-27T22:08:57.000Z
authors:
  - title: Adam Cogan
    url: https://ssw.com.au/people/adam-cogan
  - title: Kaique Biancatti
    url: https://ssw.com.au/people/kaique-biancatti
related: []
redirects:
  - do-you-know-the-best-way-to-find-your-or-others-recent-files
  - do-you-know-the-best-way-to-find-your-(or-others)-recent-files
lastUpdated: 2020-07-27T22:19:08.000Z
lastUpdatedBy: TiagoAraujo
lastUpdatedByEmail: TiagoAraujo@ssw.com.au
createdBy: TiagoAraujo
createdByEmail: TiagoAraujo@ssw.com.au
---

In Bing, if you are logged in to your work account, you can easily see your recent files!
The same works for your work colleagues.
<figureEmbed figureEmbed={{
  preset: "goodExample",
  figure: '![Good Example: Bing - Find your files faster by just searching on Bing](bing-recent-files.png)',
  shouldDisplay: true
} } />


**Suggestion to Microsoft:** Put a checkbox "Show thumbnails" like Delve.

In a slower way, you can go to Delve to see this information with a different UI with big images.

<figureEmbed figureEmbed={{
  preset: "badExample",
  figure: '![Bad Example: Delve - Is harder to get to and the UI doesn\'t offer a summary view like Bing](delve-recent-files.png)',
  shouldDisplay: true
} } />

...
There's no index, no embedding pipeline, no sync job, that you would have to set up as with RAG. It's also the only strategy that reasons across the whole corpus at once.
...
RAG is an architecture where a user query and documents are converted into numerical embedding vectors in order to match natural language queries to semantically similar content.
...
<imageEmbed alt="Decision tree for choosing a data access strategy" size="large" showBorder={false} figurePrefix="none" figure="Choosing a data access strategy by corpus size, reasoning scope, and whether the documents repeat" src="/uploads/rules/get-data-to-your-ai/data-access-decision-tree-v2.svg" />
...
| **Cost driver**    | Re-sent every request - caching only helps if the documents repeat                           | Paid once, at indexing time… typically **the cheapest option** | Tokens and turns while the agent searches            |

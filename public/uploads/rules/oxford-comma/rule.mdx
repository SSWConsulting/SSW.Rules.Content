---
authors:
- title: Lee Hawkins
  url: https://www.ssw.com.au/people/lee-hawkins
categories:
- category: categories/design/rules-to-better-content-design.mdx
created: 2022-09-29 00:15:57+00:00
createdBy: Lee Hawkins [SSW]
createdByEmail: 106290015+leehawkinsssw@users.noreply.github.com
guid: 0bc2f5a6-31ab-41fd-9951-5af483824b46
isArchived: false
lastUpdated: 2024-08-13 05:47:12+00:00
lastUpdatedBy: Matthew Parker [SSW]
lastUpdatedByEmail: 61717342+MattParkerDev@users.noreply.github.com
related:
- rule: public/uploads/rules/are-you-careful-with-your-spelling-grammar-and-punctuation/rule.mdx
seoDescription: Do you know how to use the Oxford comma? Learn the rule and when to
  add that extra comma to avoid ambiguity in your writing.
title: Do you know how to use the Oxford comma?
type: rule
uri: oxford-comma
---

The "Oxford comma" (so-called because the Oxford University Press style guidelines require it) has the distinction of being one of the most hotly debated elements of the English language.


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  caption={"Some people love Oxford commas, some hate them"}
  src="/uploads/rules/oxford-comma/oxford-comma-debate.jpg"
/>


Knowing when to use the Oxford comma helps to create more consistent and easier to read documentation.

Also referred to as a series comma or serial comma, an Oxford comma is placed in a series of three or more items before the conjunction. It can be used in both "and" and "or" lists as the last comma separating a series of items. It works to help order these items and provide a distinction between the items on the list, particularly the last two items. An Oxford comma is often unnecessary, though.

<endOfIntro />


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  caption={"A sentence with quite different meaning depending on whether an Oxford comma is used or not"}
  src="/uploads/rules/oxford-comma/oxford-comma.jpg"
/>


### The rule is that a comma should only be used between the last two items of a list if it removes potential ambiguity, otherwise no comma is required

Let's look at some examples to illustrate the rule in action.


<boxEmbed
  style="greybox"
  body={<>
    Adam, Matt, and Lee got on a call to discuss the use of the Oxford comma
  </>}
  captionStyle="badExample"
  caption={"Bad example - the extra comma after \"Matt\" is unnecessary and just adds clutter"}
/>



<boxEmbed
  style="greybox"
  body={<>
    The ice cream comes in an assortment of flavours including banana and strawberry, strawberry and mango, and blueberry.
  </>}
  captionStyle="goodExample"
  caption={"Good example - without the Oxford comma, it's not clear whether the flavour is strawberry and mango or mango and blueberry"}
/>


This short TED Ed talk covers the topic well.

<youtubeEmbed url="https://youtu.be/ptM7FzyjtRk" description={""} />
---
archivedreason: Silverlight is no longer installable and has been deprecated for 10 years.
authors:
- title: Adam Cogan
  url: https://ssw.com.au/people/adam-cogan
created: 2011-06-22 04:58:14+00:00
createdBy: SSW2000\MohammadChowdhury
...
redirects: []
seoDescription: When do you use Silverlight? Silverlight is ideal for creating a richer
  experience, but it's not suitable for data entry forms or Android and iOS users.
  Instead, HTML 5 should be the default experience for business apps.
title: Do you know when to use Silverlight?
categories:
...
YES. Using Silverlight Rich TextBox user can format Text, insert image, DataGrid, Calendar, display Text right-to-left, print content, and can access clipboard. 

## For mapping apps

YES. Microsoft.Maps.MapControl provides functionality to access Bing Maps. Using script a site can implement map feature by referencing the Bing Maps Silverlight Control XAP file in an object tag.

## Slow Response

The initial slow response is due to downloading of the .xap file. This is not an issue since it is a one-time download and the user gets a fast response with later interaction. For later visits, the user will not face a slow response issue as the browser already cached the data.

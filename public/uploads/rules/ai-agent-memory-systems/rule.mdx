...
<boxEmbed
  style="info"
  body={<>
    Note: tool-local memory is useful, but it isn't deterministic. The tool decides what's worth remembering, and different versions behave differently. Audit it now and then — it's machine-local context, so it can drift or capture something you'd rather move into repo instructions. For anything you can't afford the agent to forget, put it in `CLAUDE.md` or `AGENTS.md` yourself.
  </>}
  figurePrefix="none"
  figure=""
/>

---
archivedreason: null
authors:
- title: Adam Cogan
  url: https://ssw.com.au/people/adam-cogan
- title: Tiago Araujo
  url: https://ssw.com.au/people/tiago-araujo
categories:
- category: categories/marketing-and-video/rules-to-better-websites-branding-and-marketing.mdx
- category: categories/software-engineering/rules-to-better-websites-graphics.mdx
created: 2023-08-22 06:22:54+00:00
createdBy: Tiago Araújo [SSW]
createdByEmail: tiagov8@gmail.com
guid: d9136030-ff29-4617-8d0b-074096ae8120
isArchived: false
lastUpdated: 2024-06-08 03:27:17+00:00
lastUpdatedBy: Brady Stroud [SSW]
lastUpdatedByEmail: bradystroud@ssw.com.au
redirects:
- do-you-have-a-favicon-in-your-webpage
related:
- rule: public/uploads/rules/does-your-sharepoint-site-have-a-favicon/rule.mdx
seoDescription: Add a high-quality favicon to your website to give it a professional
  look and feel, enhancing user experience.
title: Do you add a favicon to your website?
type: rule
uri: favicon
---

A Favicon is an image file included on professionally developed sites. The favicon reflects the look and feel of the website or the organizations' visual identity.

<endOfIntro />

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="badExample"
  caption={"Bad example - When you don't add a favicon the user sees the generic browser's icon"}
  src="/uploads/rules/favicon/favicon-bad.jpg"
/>


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="goodExample"
  caption={"Good example - Using the favicon gives your website professional look and feel"}
  src="/uploads/rules/favicon/favicon-good.jpg"
/>


### Which formats and sizes to use?

The format of the image must be one of PNG (a W3C standard), GIF, or ICO. You can export your favicon in all necessary sizes on [Favicon Generator website](https://realfavicongenerator.net).

### How to implement the favicon?

1. Copy your company's favicon to the root of the site
2. Add the highlighted code below inside the `<head>` tag in your HTML

```html
<head>
  <title>Page Title</title>
  <link rel="shortcut icon" href="/images/favicon.ico" type="image/x-icon" />
</head>
```

**Figure: One line of HTML lets you add your company's icon to your web page**

This works for most websites, including ASPX WebForms, MVC and WordPress.
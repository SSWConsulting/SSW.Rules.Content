...
<imageEmbed
  ...
  showBorder={false}
  figurePrefix="good"
  figure="Build Passing with no summary issues"
...
<imageEmbed
  ...
  showBorder={false}
  figurePrefix="good"
  figure="Steps added to build definition"
...
If your build is being hosted, then the config file must be reloaded every time. <mark style="background-color: #FEF08A">If your build is running on premises</mark>, the config file will attempt to load over the existing one and break the build.

---
archivedreason: null
authors:
- title: Adam Cogan
  url: https://ssw.com.au/people/adam-cogan
- title: Matt Goldman
  url: https://ssw.com.au/people/matt-goldman
categories:
- category: categories/project-delivery/rules-to-better-scrum-using-github.mdx
- category: categories/project-delivery/rules-to-better-scrum.mdx
created: 2020-09-24 03:52:47+00:00
createdBy: Unknown
createdByEmail: Unknown@ssw.com.au
guid: de9846d4-5b51-417b-afdc-946cc096243e
isArchived: false
lastUpdated: 2024-06-08 03:27:17+00:00
lastUpdatedBy: Brady Stroud [SSW]
lastUpdatedByEmail: bradystroud@ssw.com.au
redirects:
- do-you-know-where-to-discuss-the-backlog
seoDescription: Do you know where to discuss the backlog?
title: Scrum – Do you know where to discuss the backlog?
type: rule
uri: discuss-the-backlog
---

When discussing a PBI/Issue, Pull Request, or a project in general, it is important to do it in the right place.

![](/uploads/rules/discuss-the-backlog/Kanban-on-Screen.jpg)

<endOfIntro />

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="badExample"
  caption={"Bad Example – don't use emails to discuss tasks"}
  src="/uploads/rules/discuss-the-backlog/bad-mention-pbi_1710232021933.jpg"
/>


### For code

Sometimes developers need to discuss code implementations - sometimes to improve the code, other times to explain why something was done a certain way.

This should be done in the Pull Request, if possible comment directly on the line of the code change and once resolved, make sure that the important information is captured in the merge's commit description.

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="goodExample"
  caption={"You can add a comment on a specific line of code"}
  src="/uploads/rules/discuss-the-backlog/comment on code in pull request.png"
/>


### For a new PBI/Issue

As per[Do you know when you use @ mentions in a PBI?](/when-you-use-mentions-in-a-pbi) - Create a new issue mentioning the Product Owner and the related people

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="goodExample"
  caption={"Good Example - When adding a GitHub issue, @ mention the Product Owner and other related people so they receive a notification e.g, an email"}
  src="/uploads/rules/discuss-the-backlog/Create-Issue.png"
/>


### For an existing PBI/Issue

Discuss it in the existing PBI/Issue.

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="goodExample"
  caption={"You can discuss an existing issue, even when it has been closed"}
  src="/uploads/rules/discuss-the-backlog/existing issue discussion.png"
/>


### For other topics (brainstorm ideas, general discussion, etc.)

You can:

- Create a PBI/Issue
  - use a "discussion" label so that others know that it is just a discussion point and not actionable work yet
  - have it checked by the client before publishing it (recommended)
- Discuss it in the discussion tab in GitHub

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="goodExample"
  caption={"You can create discussions on your project using the Discussion tab in your GitHub repository"}
  src="/uploads/rules/discuss-the-backlog/Discussion Tab in GitHub.png"
/>


- In the team channel in Teams

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="goodExample"
  caption={"You can discuss your idea in Teams on the team channel"}
  src="/uploads/rules/discuss-the-backlog/Team channel.png"
/>


In summary, Email should be the last resort.
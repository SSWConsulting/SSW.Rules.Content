---
archivedreason: Replaced by https://www.ssw.com.au/rules/website-page-speed (using
  Google's standard page instead of a browser add-on)
authors:
- title: Adam Cogan
  url: https://ssw.com.au/people/adam-cogan
- title: Barry Sanders
  url: https://ssw.com.au/people/barry-sanders
- title: Shane Ye
  url: https://ssw.com.au/people/shane-ye
categories:
- category: categories/software-engineering/rules-to-better-internationalization.mdx
created: 2019-05-30 04:52:23+00:00
createdBy: Isaac Lu
createdByEmail: IsaacLu@ssw.com.au
guid: 0db8cbaf-1906-45e5-bc4f-73751a27dade
isArchived: false
lastUpdated: 2021-03-15 23:43:14.765000+00:00
lastUpdatedBy: Tiago Araújo [SSW]
lastUpdatedByEmail: tiagov8@gmail.com
redirects: []
seoDescription: PageSpeed Insights can significantly optimize your webpages' performance
  with actionable suggestions.
title: Do you use PageSpeed?
type: rule
uri: do-you-use-pagespeed
---

PageSpeed can analyze the performance of your webpages and get specific suggestions on how to optimize them.

<endOfIntro />

## How to install PageSpeed?

Install PageSpeed from [Chrome web store](https://chrome.google.com/webstore/detail/pagespeed-insights-with-p/lanlbpjbalfkflkhegagflkgcfklnbnh?hl=en-GB).


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  captionText={"Install PageSpeed Insights (with PNaCl)"}
  src="/uploads/rules/do-you-use-pagespeed/2d.png"
/>


## How to use PageSpeed?

In Developer Tools (F12), you will have a new tab named PageSpeed.


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  captionText={"Start your PageSpeed by clicking \"Analyze\""}
  src="/uploads/rules/do-you-use-pagespeed/use-pageSpeed.png"
/>


The result of the analysis will be displayed in the PageSpeed tab.


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  captionText={"Some files can be compressed more than 80%"}
  src="/uploads/rules/do-you-use-pagespeed/compressed-result.png"
/>
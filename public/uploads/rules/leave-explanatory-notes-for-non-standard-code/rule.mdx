...
```
  ...
  caption={"Bad Example - This format is ambiguous. Is it a permanent note or a temporary reminder for the author to fix later?"}
/>
...
...To ensure idempotent and reliable local test runs, the database for the 'clean-architecture' project must be dropped and recreated on each execution when running in `DEBUG` mode. This prevents data contamination between test sessions...
  ...
  captionStyle="badExample"
  caption={"Bad Example - Documentation is disconnected from the code. A developer won't see this unless they know to look for it, making it easy to miss."}
/>
...
```
  ...
  caption={"Good Example - The `NOTE:` prefix makes the intent clear. The comment is permanent, explains the deviation, and provides a link for more context."}
/>

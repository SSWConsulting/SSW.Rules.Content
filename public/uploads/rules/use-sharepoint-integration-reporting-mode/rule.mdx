---
archivedreason: null
authors:
- title: Jeoffrey Fischer
  url: https://ssw.com.au/people/jeoffrey-fischer
created: 2024-09-16 09:12:00+00:00
guid: c5baeed9-b836-40f7-9039-75be9612fbc9
redirects: []
related:
- rule: public/uploads/rules/when-to-use-reporting-services/rule.mdx
seoDescription: Enhance your reporting experience with SharePoint Integration Mode
  for easier deployment and improved security compared to Native Reporting Mode.
title: Do you use SharePoint Integration Reporting Mode over Normal(Native) Reporting
  Mode?
categories:
  - category: categories/company-operations/rules-to-better-reporting-services.mdx
type: rule
uri: use-sharepoint-integration-reporting-mode
---

With the use of SharePoint 2010 Integration mode of Reporting, users can have the following advantages:

<endIntro />

The users can easily deploy data sources, reports to sharePoint document libraries instead of Report Manager.
The users can be much more self-sufficient with SharePoint.
Very easy one step configuration of the add-in
37 languages supported including bi-directional languages.
Accessing Reporting in local mode when Access Services is enabled.
Improved Report Preview experience with Report Builder 3.0 and edit sessions and deploy their reports to SharePoint document libraries, leveraging SharePoint for security.
The users can take advantage of the new version of Report Builder that came with SQL Server 2008 R2 and deploy their reports to SharePoint document libraries, leveraging SharePoint for security.

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="bad"
  figure="SQL Report Manager (which requires Visual Studio and TFS if you want source control)"
  src="/uploads/rules/use-sharepoint-integration-reporting-mode/NativeMode.jpg"
/>


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="good"
  figure="SharePoint Integration (you get nice source control via SharePoint and you can use the nice Report Builder 3)"
  src="/uploads/rules/use-sharepoint-integration-reporting-mode/SharePointIntegratedMode.jpg"
/>
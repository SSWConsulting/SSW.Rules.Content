---
archivedreason: null
authors:
- title: Adam Cogan
  url: https://ssw.com.au/people/adam-cogan
categories:
- category: categories/infrastructure-and-networking/rules-to-better-hyper-v-clustering.mdx
created: 2012-03-02 18:47:12+00:00
createdBy: TiagoAraujo
createdByEmail: TiagoAraujo@ssw.com.au
guid: 9a9c8925-bfc8-4463-ae14-2b9daa471a3e
isArchived: false
lastUpdated: 2024-06-08 03:27:17+00:00
lastUpdatedBy: Brady Stroud [SSW]
lastUpdatedByEmail: bradystroud@ssw.com.au
redirects: []
seoDescription: Consistently manage cluster node settings using group policy to ensure
  uniformity across all nodes.
title: Do you use group policy to apply settings to all of your cluster nodes?
type: rule
uri: do-you-use-group-policy-to-apply-settings-to-all-of-your-cluster-nodes
---

Don't log in and make manual changes to the clustered nodes.

When working with clustered environments it is important that settings be consistent across every node. The best way to handle this is through group policy.

<endOfIntro />

Create a policy that you would like applied to each node of the cluster using the **Group Policy Management** .


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  caption={"Bad example - Do not manually change settings on each node"}
  src="/uploads/rules/do-you-use-group-policy-to-apply-settings-to-all-of-your-cluster-nodes/group-policy-bad.jpg"
/>



<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  caption={"Good example - Changing settings through Group Policy keeps node settings the same"}
  src="/uploads/rules/do-you-use-group-policy-to-apply-settings-to-all-of-your-cluster-nodes/group-policy-good.jpg"
/>
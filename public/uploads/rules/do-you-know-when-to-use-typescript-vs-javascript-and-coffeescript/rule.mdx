---
archivedreason: null
authors:
- author: eric-phan
created: 2013-08-27 17:14:42+00:00
createdBy: Tiago Araujo
createdByEmail: TiagoAraujo@ssw.com.au
guid: f22bd76c-6173-45ab-ae3c-3bc0eabb06ff
isArchived: false
lastUpdated: 2015-08-24 02:02:53+00:00
lastUpdatedBy: William Yin
lastUpdatedByEmail: WilliamYin@ssw.com.au
redirects:
- do-you-know-when-to-use-typescript-(vs-javascript-and-coffeescript)
seoDescription: Microsoft's TypeScript compiles into JavaScript, ideal for reusable
  client-side libraries, while JavaScript suits page-specific scripts and CoffeeScript's
  syntax differs too much from JavaScript.
title: Do you know when to use TypeScript (vs JavaScript and CoffeeScript)?
categories:
- category: categories/software-engineering/rules-to-better-javascript-and-jquery.mdx
type: rule
uri: do-you-know-when-to-use-typescript-vs-javascript-and-coffeescript
---
TypeScript is the new flagship language from Microsoft that compiles into JavasScript.

- Use JavaScript if you’re writing page specific script
- Use jQuery to improve cross-browser support
- Use TypeScript if you’re writing re-usable client side library

Don't use CoffeeScript (language is too different from JavaSript)

<endIntro />

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="good"
  figure="TypeScript is very similar to JavaScript so it’s easy to learn, and also helps developers learn proper JavaScript coding practices"
  src="/uploads/rules/do-you-know-when-to-use-typescript-vs-javascript-and-coffeescript/typescript-good.jpg"
/>


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="bad"
  figure="CoffeeScript syntax is very different from JavaScript so it takes longer to learn"
  src="/uploads/rules/do-you-know-when-to-use-typescript-vs-javascript-and-coffeescript/typescript-bad.jpg"
/>
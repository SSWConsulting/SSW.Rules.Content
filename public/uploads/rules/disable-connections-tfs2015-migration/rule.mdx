---
archivedreason: null
authors:
- title: Adam Cogan
  url: https://ssw.com.au/people/adam-cogan
categories:
- category: categories/project-delivery/rules-to-better-tfs-2015-migration.mdx
created: 2021-12-07 02:59:09+00:00
createdBy: Tiago Araújo [SSW]
createdByEmail: tiagov8@gmail.com
guid: 1071b5f7-6efa-4e36-9987-39c04725a008
isArchived: false
lastUpdated: 2024-06-08 03:27:17+00:00
lastUpdatedBy: Brady Stroud [SSW]
lastUpdatedByEmail: bradystroud@ssw.com.au
redirects:
- do-you-disable-connections1
seoDescription: Disable connections to prevent changes during TFS 2015 migration,
  ensuring data integrity and minimizing losses.
title: Do you disable connections for TFS 2015 migration?
type: rule
uri: disable-connections-tfs2015-migration
---

It is important that while you're upgrading, nobody can check in. Any check-ins after you backup your database will be lost.

To make sure that nobody can change anything during the upgrade, follow these steps.

<endOfIntro />

a. Send out an email notifying everyone TFS will be unavailable for the upgrade period

b. Make sure nobody can check in files:

c. Open the TFS Administration Console on the server.

d. Navigate to Application Tier / Team Project Collections.

e. For each Team Project Collection, select it, and click "Stop Collection". Enter a useful message (this will be displayed to users trying to connect from Visual Studio) and click "Stop":


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  captionText={"Stop each Team Project Collection"}
  src="/uploads/rules/disable-connections-tfs2015-migration/stop each term.png"
/>



<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  captionText={"All Team Project Collections are stopped"}
  src="/uploads/rules/disable-connections-tfs2015-migration/all team project.png"
/>


f. In Visual Studio, confirm you can no longer connect to TFS


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  captionStyle="default"
  captionText={"Visual Studio shows the message that you entered when you stopped the Team Project Collection"}
  src="/uploads/rules/disable-connections-tfs2015-migration/visual studio.png"
/>
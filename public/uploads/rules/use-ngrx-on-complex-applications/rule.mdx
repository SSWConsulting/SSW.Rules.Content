...
seoDescription: Practices - Do you know to use NgRx on complex applications? NgRx
  helps manage complexity by storing state and handling side effects. Perfect for
  large-scale projects like shopping carts where multiple components need to stay
  in sync.
...
<imageEmbed
  ...
  captionText={"NgRx supercharges the redux pattern with RxJS"}
...
<imageEmbed
  ...
  showBorder={false}
  captionStyle="default"
  captionText={"The redux pattern is a way to implement a predictable state container"}
...
<imageEmbed
  ...
  showBorder={false}
  captionStyle="default"
  captionText={"NGRX pattern with Effect"}

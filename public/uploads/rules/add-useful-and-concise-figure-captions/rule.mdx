---
type: rule
title: Do you add useful text captions to images and videos?
uri: add-useful-and-concise-figure-captions
categories:
  - category: categories/design/rules-to-better-content-design.mdx
authors:
  - title: Adam Cogan
    url: 'https://www.ssw.com.au/people/adam-cogan'
  - title: Tiago Araujo
    url: 'https://www.ssw.com.au/people/tiago-araujo'
related:
  - rule: public/uploads/rules/screenshots-avoid-walls-of-text/rule.mdx
  - rule: public/uploads/rules/use-bad-and-good-examples/rule.mdx
  - rule: public/uploads/rules/use-the-right-html-figure-caption/rule.mdx
  - rule: public/uploads/rules/right-format-to-write-videos-time-length/rule.mdx
  - rule: public/uploads/rules/placeholder-for-replaceable-text/rule.mdx
redirects:
  - >-
    figures-do-you-add-useful-and-concise-figure-text-(aka-a-caption)-to-avoid-a-lot-of-text-over-images
  - add-a-useful-figure-caption-below-all-images
  - show-a-bad-example-and-then-a-good-example
  - figures-do-you-add-useful-and-concise-figure-text
guid: cb664ecb-4910-4d82-bcbc-47e35cbe89ee
seoDescription: >-
  Enhance your web content by adding useful figure captions to images and videos
  for better SEO and user engagement.
created: 2014-12-04T20:25:39.000Z
createdBy: Tiago Araujo
createdByEmail: TiagoAraujo@ssw.com.au
lastUpdated: 2026-01-15T05:03:30.925Z
lastUpdatedBy: Hark
lastUpdatedByEmail: harksingh@ssw.com.au
isArchived: false
archivedreason: null
---

When you add images/videos/tables to websites/applications, it is helpful to add a caption underneath them, describing and including extra information to users.

It's a convenient way of catching users' attention to your content. When people are scanning a newspaper, they often check out the pictures first, then read the accompanying description, and if it sounds interesting, they'll go back and read the article. Users read websites in a similar fashion.

<endIntro />

To catch readers' attention, include a useful description - don't just describe what the image is... say what it's used for in the context of the document.

It is especially important that images and captions serve a purpose, as opposed to graphics which are there solely for design.

## Tip 1: Use prefixes

Prefix your caption with "Figure: ", "Table: ", "Video: ", "Code: ", or "GIF: "

If it is a good/ok/bad example (see the next tip), then the prefix should be something like: "Figure: Good/Bad/OK example - ", "Video: Good/Bad/OK example - ", or "Code: Good/Bad/OK example - ".

<boxEmbed
  style="info"
  body={<>
    **Note:** The first word after the dash should be capitalized, and the [caption should not include a full stop](/avoid-full-stops-in-bullet-point-lists/) at the end.
    
    E.g. **Figure: Good example - This is a caption**
  </>}
  figurePrefix="none"
  figure=""
/>

## Tip 2: Give bad and good examples

When possible, use "bad" and "good" examples to clearly explain **Dos and don'ts**.

<boxEmbed
  style="info"
  body={<>
    At SSW we always show the bad example first, then the good example. You will see samples of this in the next tips below.
  </>}
  figurePrefix="none"
  figure=""
/>

## Tip 3: Bold your captions

<boxEmbed
  style="greybox"
  body={<>
    {{ IMAGE }}\
    {{ CAPTION }}
  </>}
  figurePrefix="bad"
  figure="Caption not bolded can be mixed up with regular content"
/>

<boxEmbed
  style="greybox"
  body={<>
    {{ IMAGE }}\
    **{{ CAPTION }}**
  </>}
  figurePrefix="good"
  figure="Caption stands out when bolded"
/>

## Tip 4: Describe the actions in your captions

Especially for screenshots, it is a good idea to have your figure describe the action the user would take:

<boxEmbed
  style="greybox"
  body={<>
    {{ IMAGE }}\
    **Figure: This is the screen**
  </>}
  figurePrefix="bad"
  figure="Vague caption description"
/>

<boxEmbed
  style="greybox"
  body={<>
    {{ IMAGE }}\
    **Figure: On the screen, choose the execution method**
  </>}
  figurePrefix="good"
  figure="Clear caption description"
/>

## Tip 5: Video captions should be the title + length

When embedding videos, include a caption with **the video title** + **video length** in brackets.

**Note:** As per Tip #1, you should also prefix with "Video: " instead of "Figure: ".

<boxEmbed
  style="greybox"
  body={<>
    {{ VIDEO }}\
    **Figure: Scrum video**
  </>}
  figurePrefix="bad"
  figure="Using &#x22;Figure:&#x22; for a video caption + a vague text"
/>

<boxEmbed
  style="greybox"
  body={<>
    {{ VIDEO }}\
    **Video: Intro to Scrum in Under 10 Minutes (9 min)**
  </>}
  figurePrefix="good"
  figure="Video caption following the standard &#x22;Video: {{ VIDEO TITLE }} ({{ VIDEO LENGTH}})&#x22;"
/>

<boxEmbed
  body={<>
    Main reason is, it acts as a reminder in case the video ever gets removed by its owner (otherwise the embed just shows “Video not found”, which is bad UX).
  </>}
  figurePrefix="none"
  figure=""
  style="tips"
/>

This also helps:

* Giving a brief text summary of the video
* Getting some extra Google Juice
* Letting users know what to expect in terms of time required to watch

<boxEmbed
  style="info"
  body={<>
    **Note:** The exception is for promotional videos where the caption may undesirably impact the look and feel of your page. If you don't include the video title in the caption, consider adding it above the video as regular content, so it's searchable.
  </>}
  figurePrefix="none"
  figure=""
/>

## Tip 6: AI images - include your prompt

If you're sharing an AI generated image, show others how you made it.

<boxEmbed
  style="greybox"
  body={<>
    {{ IMAGE }}\
    **Figure: An AI image of baseball player sliding**
  </>}
  figurePrefix="bad"
  figure="Doesn't educate others about AI images"
/>

<boxEmbed
  style="greybox"
  body={<>
    {{ IMAGE }}\
    **Figure: An AI image of baseball player sliding. Prompt: A realistic image of a baseball player sliding. The uniform is white and the crowd is cheering.**
  </>}
  figurePrefix="good"
  figure="Readers understand how you made the image, and they improve their own AI image skills by learning from your caption"
/>

## Tip 7: Link people's names to their profiles

When you have someone's name in your caption, link the name to their profiles (e.g. [SSW People profile](https://www.ssw.com.au/people)).

<boxEmbed
  style="greybox"
  body={<>
    {{ VIDEO }}\
    **Video: In this video, Bob talks about Outlook**
  </>}
  figurePrefix="bad"
  figure="A vague text with no link to Bob's profile. Also missing the video length"
/>

<boxEmbed
  style="greybox"
  body={<>
    {{ VIDEO }}\
    **Video: How to search on Outlook by [Bob](https://www.ssw.com.au/people/sample) (2 min)**
  </>}
  figurePrefix="good"
  figure="A descriptive caption using the video title + profile link + video length at the end"
/>

## Tip 8: GIFs - Label accordingly

Using a GIF instead of a static image can be beneficial for illustrating multiple steps, as it saves page space. Be sure to specify that it is a GIF in the caption to distinguish it from a static image

<boxEmbed
  style="greybox"
  body={<>
    {{ GIF }}\
    **Figure: Users | Summary | User Information | LinkedIn URL**
  </>}
  figurePrefix="bad"
  figure="Does not specificy that it is a GIF"
/>

<boxEmbed
  style="greybox"
  body={<>
    {{ GIF }}\
    **Figure: Animated GIF - Users | Summary | User Information | LinkedIn URL**
  </>}
  figurePrefix="good"
  figure="Specificies that it is a GIF"
/>

## Tip 9: Tables - lead with the takeaway

A table is data, not a picture. Readers scan the caption before they read the rows - so if the data says something, say it in the caption.

<boxEmbed
  style="greybox"
  body={<>
    {{ TABLE }}\
    **Table: AI tool usage for Sprint 12**
  </>}
  figurePrefix="bad"
  figure="Repeats the heading and tells the reader nothing the table doesn't already show"
/>

<boxEmbed
  style="greybox"
  body={<>
    {{ TABLE }}\
    **Table: 3 of 5 devs hit Claude limits this sprint - worth reviewing plan tiers**
  </>}
  figurePrefix="good"
  figure="The reader gets the finding without reading a single row"
/>

<boxEmbed
  style="info"
  body={<>
    **Note:** If the data has no takeaway this time, describe it plainly. Never invent a finding just to fill the caption.
  </>}
  figurePrefix="none"
  figure=""
/>

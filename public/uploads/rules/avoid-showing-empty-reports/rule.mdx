...
seoDescription: Avoid empty reports by using database queries to set valid default
  parameter values in SQL Reporting Services, ensuring meaningful results for users.
...
<imageEmbed
  ...
  captionText={"Bad example - Making a user select the parameters before seeing the data"}
...
<imageEmbed
  ...
  showBorder={false}
  captionStyle="badExample"
  captionText={"Bad example - Empty report caused by incorrect parameter default values (probably hard coded for when the developer wrote the report in 2006)"}
...
<imageEmbed
  ...
  captionText={"Good example - This report shows initial data as the developer configure useful parameters (in this case default values for the entire month of October Note: in US date format)"}
...
<imageEmbed
  ...
  showBorder={false}
  captionStyle="badExample"
  captionText={"Bad example - Using an Expression to set the default values.(This will not be good enough if there is no data in the current month)"}
...
<imageEmbed
  ...
  showBorder={false}
  captionStyle="goodExample"
  captionText={"Good example - Using a query to set default values for report parameters"}
...
<imageEmbed
  ...
  captionText={"Create a dataset to query the data and provide useful default parameter values for your report"}

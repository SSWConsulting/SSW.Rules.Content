---
seoDescription: PowerBI data visualization tips - Avoid alphabetical sorting, which is limited and lacks insight into ranks, trends, or proportions.
type: rule
title: Do you know not to use Alphabetical Sorting?
uri: do-you-know-not-to-use-alphabetical-sorting
authors:
  - title: Ulysses Maclaren
    url: https://ssw.com.au/people/ulysses-maclaren
related: []
redirects: []
created: 2016-10-21T05:14:11.000Z
archivedreason: null
guid: 3d916e30-78b6-4bec-b3a8-394e227fa148
lastUpdated: 2024-06-08T03:27:17.000Z
lastUpdatedBy: Brady Stroud [SSW]
lastUpdatedByEmail: bradystroud@ssw.com.au
createdBy: ulyssesmaclaren
createdByEmail: ulyssesmaclaren@ssw.com.au
isArchived: false
---

<introEmbed
  body={<>
The default sort order for most visualizations is alphabetical, but you should almost always change this.
  </>}
/>
<figureEmbed figureEmbed={ {
  preset: "badExample",
  figure: "Alphabetical sorting is really only good if you're specifically looking for one item on the X axis, but it's terrible for seeing ranks, trends, or proportions.\n\n<imageEmbed\n  alt=\"Image\"\n  size=\"large\"\n  showBorder={false}\n  figureEmbed={ {\n    preset: \"default\",\n    figure: \"Bad Example: Alphabetical gives very little information\",\n    shouldDisplay: true\n  } }\n  src=\"/uploads/rules/do-you-know-not-to-use-alphabetical-sorting/PowerBI-alphabetical.png\"\n/>",
  shouldDisplay: true
} } />


<figureEmbed figureEmbed={ {
  preset: "goodExample",
  figure: "![Figure - Good Example: Sorting by value shows rank and trends easily](/uploads/rules/do-you-know-not-to-use-alphabetical-sorting/PowerBI-non-aphabetical.png)",
  shouldDisplay: true
} } />

---
archivedreason: Outdated - This references the Web Platform Installer (discontinued) and TFS-era Web Deploy setup guidance. For modern deployment guidance, see [Do you use the best deployment tool?](/do-you-use-the-best-deployment-tool) and [DevOps best practices](/rules-to-better-devops).
authors:
- title: Adam Stephensen
  url: https://www.ssw.com.au/people/adam-stephensen
created: 2013-02-06 18:39:41+00:00
createdBy: Tiago Araujo
createdByEmail: TiagoAraujo@ssw.com.au
guid: 44ecb0e3-2e08-4fee-9eec-84e3cb192a84
isArchived: true
lastUpdated: 2013-06-03 19:20:03+00:00
lastUpdatedBy: Tiago Araujo
lastUpdatedByEmail: TiagoAraujo@ssw.com.au
redirects: []
seoDescription: Don't install Web Deploy from the Web Platform Installer; instead,
  download the package from IIS to ensure all necessary components for continuous
  deployment are included.
title: Do you not install Web Deploy from the Web Platform Installer?
categories:
  - category: categories/project-delivery/rules-to-better-continuous-deployment-with-tfs.mdx
type: rule
uri: do-you-not-install-web-deploy-from-the-web-platform-installer
---

You should not Install Web Deploy from the Web Platform Installer, but instead download the installation from the IIS website (http://www.iis.net/downloads/microsoft/web-deploy).

The reason for this is that the Web Platform Installer does not install all of the components required for continuous deployment, but the downloaded package does.

<endIntro />

More information on this issue here: [Don't Install Web Deployment Tool using the Web Platform Installer](http://nicksnettravels.builttoroam.com/post/2010/04/22/Done28099t-Install-Web-Deployment-Tool-using-the-Web-Platform-Installer.aspx)

<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="bad"
  figure="Installing Web Deploy from the Web Platform Installer does not install all the components required for continuous deployment"
  src="/uploads/rules/do-you-not-install-web-deploy-from-the-web-platform-installer/web-platform-installer.jpg"
/>


<imageEmbed
  alt="Image"
  size="large"
  showBorder={false}
  figurePrefix="good"
  figure="Install Web Deploy 3.0 by downloading the package from        http://www.iis.net/downloads/microsoft/web-deploy"
  src="/uploads/rules/do-you-not-install-web-deploy-from-the-web-platform-installer/web-deploy-installer.jpg"
/>
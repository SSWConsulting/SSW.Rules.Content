    ...
    - name: Guard — only ContentHawk judge issues
      ...
      run: |
        if echo "$BODY" | grep -qF "gh-aw-workflow-id: content-judge"; then
          ...
          echo "is_contenthawk_judge_issue=true" >> "$GITHUB_OUTPUT"
        else
          echo "Not a ContentHawk judge issue. Skipping workflow."
          echo "is_contenthawk_judge_issue=false" >> "$GITHUB_OUTPUT"
        fi
...
if: github.event_name != 'issues' || needs.pre_activation.outputs.is_contenthawk_judge_issue == 'true'

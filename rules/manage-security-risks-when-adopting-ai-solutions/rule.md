...
* **Opt-out of training public models:** When using enterprise versions of models, such as OpenAI services on Azure, data used for analysis is automatically opted out of model training. Ensure that you are opted out of model training for any enterprise data, either by default (e.g. with OpenAI on Azure) or manually if necessary (e.g. with GitHub Copilot).

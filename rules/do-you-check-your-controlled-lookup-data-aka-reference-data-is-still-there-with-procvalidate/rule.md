...
[Controlled Lookup Data](/rules/do-you-deploy-controlled-lookup-data) is when data is tightly coupled to the application. If the data is not there, you have problems. So how do we check to see if data is still there?
...
```
     ...
     Figure: Implement a stored procedure to check the 'Controlled Lookup Data' does not go missing   Note: As this procedure will be [executed many times, it must be Idempotent](/rules/do-you-ignore-idempotency)

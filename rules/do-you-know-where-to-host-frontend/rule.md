...
When building frontend application for your API, there's a point where the team need to choose where and how to host the frontend application.
Choosing the right hosting option is critical to scalability, available features, and flexibility of the application.

<!--endintro-->

There's two primary way to serve your frontend (e.g. Angular) application depending on where the frontend is hosted:

- **Option 1: Integrated with backend** 
    
    Frontend is served by the backend API. Both frontend and backend is hosted on the same server.
    ...
    - ❌ Difficult to scale
    - ❌ Complicated development workflow for multi-team project
    - ❌ Backend deployment is tied to frontend deployment


- **Option 2: Externally hosted**

    Frontend is served standalone on its own separate host. Backend is hosted on a different server.

    Pros & Cons:
    - 🟢 Highly scalable
    - 🟢 Access to CDN
    - 🟢 Reduced backend load
    - 🟢 Flexible technology option - e.g. swapping technology is easier
    - ❌ More complicated deployment
    - ❌ Higher cost

Based on the pros and cons of each options above, the recommended options are:

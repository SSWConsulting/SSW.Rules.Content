...
---
Doing version control with Power BI reports used to be problematic. The primary way of doing this was to commit the pbix file into the repository. But this has the following drawbacks. 
- Data also gets saved to source control, which is bad as it could be large 
...
- Git integration in Microsoft Fabric 
   * _Requires either Fabric capacity or a Power BI Premium per User license_
   * _Currently only integrates with Git repos in Azure DevOps_
...
* A Project does not by default contain any underlying data. So when you open a definition.pbir file the visuals will show as empty. Once you refresh the report Power BI Desktop will download a copy of the data into a file called cache.abf which gets stored in a ".pbi" folder inside the Dataset folder. This file should not be saved in version control. So you can create a .gitignore file to prevent Git from committing it to the repository. 
